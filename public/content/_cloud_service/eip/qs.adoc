...
本文以 IPv4 公网为例，如果您需要支持 IPv6 公网，请参见 [IPv6 公网 IP 操作指导]。
//link:../manual/ipv6/overview/[IPv6 公网 IP 操作指导]。

== 前提条件

* 已[注册云平台账号]，且已完成[实名认证]。
//link:../../../authorization/account/manual/user_signup/[注册云平台账号]，且已完成link:../../../authorization/account/manual/account_aut/account_intro/[实名认证]。
* 账号余额足够。 
* 已[创建云服务器]，云服务器需要与 EIP 在同一区域。
//link:../../../compute/vm/quickstart/create_vm/[创建云服务器]，云服务器需要与 EIP 在同一区域。
