...
+
--
image::/images/dmp/os-dashboards.png[]
--
...
下面举例如何将生成的证书绑定在 all_access 和 dashboard_read_only 两个 role 上。关于如何生成证书，请参阅link:#_生成客户端证书[生成客户端证书]。
...
+
--
image::/images/dmp/multiuser-access.png[]
--
...
+
--
image::/images/dmp/all-access-success.png[]
--

.. 使用 dashboard_read_only 证书访问 Dashboard，如果出现以下信息，则代表访问集群成功。
+
--
image::/images/dmp/dashboard-only-success.png[]
--

.. 使用 dashboard_read_only 证书访问集群。如果出现报错，则符合预期。
+
--
image::/images/dmp/dashboard-only-fail.png[]
--