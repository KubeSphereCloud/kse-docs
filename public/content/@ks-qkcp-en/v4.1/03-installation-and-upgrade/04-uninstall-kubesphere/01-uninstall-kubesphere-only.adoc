...
. Log in to any cluster node and execute the following command to view all installed extensions in the cluster.
+
[,bash]
----
...
Execute the following command again. If it outputs "No resources found", it means that all extensions have been uninstalled.
...
. Execute the following command. If the output is empty (as shown below), it means that KubeSphere Enterprise has been successfully uninstalled.
+
--
[,bash]
----
