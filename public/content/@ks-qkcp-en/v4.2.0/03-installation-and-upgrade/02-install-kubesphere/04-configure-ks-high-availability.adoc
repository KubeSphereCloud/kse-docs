...
When installing or upgrading {ks_product-en}, append `-f values.yaml` to your command.
...
. Verify configuration: Access `http://<ks-console-address>/oauth/keys` multiple times in your browser and verify the data consistency across all replicas.
