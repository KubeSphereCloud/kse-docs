...
This section only introduces the monitoring functions in a single-cluster environment. For alerting functions in a single-cluster environment, see link:../07-alerting[WhizardTelemetry Alerting]. For monitoring and alerting functions in a multi-cluster environment, see link:../08-whizard[Whizard Observability Center].
