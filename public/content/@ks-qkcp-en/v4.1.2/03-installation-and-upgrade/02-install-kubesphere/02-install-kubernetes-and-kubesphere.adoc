...
description: "Get how to install Kubernetes and KubeSphere Enterprise."
weight: 02
---
...
* Contact the {ks_product-en} support team to obtain the {ks_product-en} v4.1.2 installation package.
...
Have a view of the {ks_product-en} installation package to proceed with the following steps.

The installation package contains the following files:

[,bash]
----
kse-all-v4.1.2-offline-linux-amd64/
├── charts
...
**config-sample.yaml** is the installation configuration file of {ks_product-en}. Please configure this file first for subsequent steps.
...
. Run the following command to initialize the private image registry.
+
====
[,bash]
----
...
. If the **spec:registry:type** parameter is set to **harbor**, run the following command to create a Harbor project.
+
--
// Bash
include::../../../../_ks_components-en/code/bash.adoc[]
...
[,yaml]
----
Pipeline[CreateclusterPipeline] Run successfully
...
Run the following command to import images to the specified private image registry.
...
[,yaml]
----
Pipeline[ArtifactImagesPushPipeline] Run successfully
----
...
|
By now, the {ks_product-en} web console only provides the core functionalities of {ks_product-en}. To use the extensions, you need to continue with the following two steps to install extensions.
|===

=== Step 2: Publish Extensions

Run the following command to import all extensions provided by {ks_product-en} into the Extensions Center of the web console.
...
After publishing the extensions, log in to the {ks_product-en} web console. You can view and install the extensions in the Extensions Center. Alternatively, follow the steps below to quickly deploy the extensions.
...
kubectl apply -f kse-extensions/whizard-telemetry-installplan.yaml
...
kubectl get installplan whizard-telemetry -w
kubectl get installplan vector -w
----
--

. Run the following commands to install specific extensions or all extensions.
+
--
[,bash]
----
...
|
* Get `{EXTENSION-NAME}` in the `kse-extensions` directory of the {ks_product-en} installation package, or refer to the picture below.
...
|
For extensions that have extension configurations modified, please do not re-run `kubectl apply -f kse-extensions/{EXTENSION-NAME}-installplan.yaml` to avoid overwriting the configuration of the extensions.
|===
--

. Run the following command and wait for the state of the extensions to change to **Installed**, which indicates a successful deployment.
+
--
[,bash]
----
