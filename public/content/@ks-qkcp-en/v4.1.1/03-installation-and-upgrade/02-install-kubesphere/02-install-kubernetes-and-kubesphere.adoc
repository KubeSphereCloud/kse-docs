...
Replace `<Kubernetes version>` with the actual version needed, for example **v1.27.4**. KubeSphere Enterprise by default supports Kubernetes v1.21~1.28.
...
KubeSphere Core (ks-core) is the core component of KubeSphere Enterprise, providing a foundational runtime environment for extensions. Once KubeSphere Core is installed, you can access the KubeSphere Enterprise Web console.
...
At this point, the {ks_product-en} web console only provides the core features of KubeSphere Enterprise.

* Before using KubeSphere Enterprise, you need to bind a KubeSphere cloud account to activate KubeSphere Enterprise. For more information, please refer to link:../03-activate-kse/[Activate KubeSphere Enterprise].

* To use extensions, you also need to subscribe to and install the extensions.