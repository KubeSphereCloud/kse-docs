---
title: "离线安装 Kubernetes 和 KubeSphere 企业版"
...
本节介绍如何离线安装 Kubernetes 和{ks_product_left}。部署环境无需访问外网，可完全离线安装 KubeSphere 企业版和扩展组件。
...
* 您需要联系{ks_product_both}的交付服务专家获取{ks_product_left} v4.1.3 安装包。
...
了解{ks_product_left} v4.1.3 的安装包内容，以便进行后续步骤。
...
. 根据激活提示点击**激活**，导入{ks_product_both}的 license。

ifeval::["{file_output_type}" == "html"]
. 此时，Web 控制台仅提供{ks_product_both}的核心功能，若要体验更多功能，还需在扩展中心link:../../../06-extension-user-guide/01-install-components/[安装扩展组件]。

endif::[]

ifeval::["{file_output_type}" == "pdf"]
. 此时，Web 控制台仅提供{ks_product_both}的核心功能，若要体验更多功能，还需在扩展中心安装扩展组件。有关更多信息，请参阅《{ks_product_right}扩展组件管理指南》。
