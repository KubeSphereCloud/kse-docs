...
. 编辑 **WizTelemetry 日志**的扩展组件配置，修改 `vector-logging` 下 opensearch 的相关配置。
+
[,yaml]
----
vector-logging:
...
. 编辑 **WizTelemetry 审计**的扩展组件配置，修改 `kube-auditing` 下 opensearch 的相关配置。
+
--
[.admon.note,cols="a"]
|===
...
|
另外，还需要启用 KubeSphere 和 Kubernetes 审计。有关更多信息，请参阅扩展中心“WizTelemetry 审计”扩展组件的详情页说明。
|===
...
. 编辑 **WizTelemetry 事件**的扩展组件配置，修改 `kube-events-exporter` 下 opensearch 的相关配置。
+
[,yaml]
----
...
. 编辑 **WizTelemetry 通知**的扩展组件配置，修改 `notification-history` 下 opensearch 的相关配置。
+
[,yaml]
----
...
. 编辑 **WizTelemetry 平台服务**的扩展组件配置，确保其中各组件配置下的 `endpoints` 与以上各个扩展组件的 `endpoints` 配置一致，以便{ks_product_both}平台能够正确查询日志数据。有关更多信息，请参阅扩展中心“WizTelemetry 平台服务”扩展组件的详情页说明。
+
