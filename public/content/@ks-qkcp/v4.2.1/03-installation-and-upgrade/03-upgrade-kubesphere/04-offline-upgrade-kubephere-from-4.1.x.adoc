...
. 如果已在 4.1.x 版本中自定义配置过 nodeShell 镜像，升级时注意在升级配置文件 `kse-v4.2.1-host-custom-values.yaml` 中按照如下方式指定 nodeShell 镜像。示例如下：
+
--
若在 v4.1.x 中配置为：
...
. 自 v4.1.3 起，以下参数已废弃，升级时注意在升级配置文件 `kse-v4.2.1-host-custom-values.yaml` 中删除该参数或者设置为 `false`。
+
[source,bash]
----
upgrade:
...
. 执行以下命令解压 v4.2.1 安装包（将 <package name> 替换为安装包的实际名称）。
+
--
[source,bash]
----
...
. 在 v4.2.1 目录下，编辑安装 KubeSphere 时创建的 **config-sample.yaml **文件，修改其中的 **registry** 部分。
+
--
[source,yaml]
----
...
. KubeSphere v4.1.3 移除了 `kse-extensions-publish`，将其整合到了 `ks-core` 的 chart 中，因此在 v4.1.2 升级至 v4.2.1 之前，应该对 KubeSphere 历史版本创建的扩展组件资源进行补丁操作。
+
[.admon.attention,cols="a"]
|===
|注意
...
|
* link:https://helm.sh/zh/docs/intro/install/[helm] 版本需要 >= 3.17.0
...
. 使用原有的 Web 控制台的 IP 地址、管理员用户名和管理员密码，并使用网页浏览器登录 v4.2.1 的 KubeSphere Web 控制台。
