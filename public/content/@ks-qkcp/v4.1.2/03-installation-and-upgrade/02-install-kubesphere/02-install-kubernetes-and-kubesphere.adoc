...
* 您需要联系{ks_product_both}的交付服务专家获取{ks_product_left} v4.1.2 安装包。
...
获取{ks_product_both}安装包后，可采用校验文件哈希值的方法验证安装包的完整性，以确保其没有被损坏。
...
[,bash]
----
certutil -hashfile C:\Users\user1\Downloads\kse-v4.1.2-offline-linux-amd64-all-20240913.tar.gz SHA256
----
--
...
[,bash]
----
sha256sum ~/Downloads/kse-v4.1.2-offline-linux-amd64-all-20240913.tar.gz
----
--
...
[,bash]
----
shasum -a 256 ~/Downloads/kse-v4.1.2-offline-linux-amd64-all-20240913.tar.gz
----
--
====
...
|如果 SHA256 不一致，您需要重新获取完整的{ks_product_both}安装包。
|===
--

== 查看安装包内容

了解{ks_product_left} v4.1.2 的安装包内容，以便进行后续步骤。

安装包包含以下文件：

[,bash]
----
kse-v4.1.2-offline-linux-amd64-all/
├── charts
...
└── manifest-v412-amd64.yaml      # KubeSphere 企业版制品清单，其中包含各组件的版本以及镜像列表
----
...
. 若要安装可观测相关扩展组件，必须先部署各组件依赖的 whizard-telemetry（WhizardTelemetry 平台服务）、vector（WhizardTelemetry 数据流水线），然后安装其他可观测组件。否则，可跳过此步骤。
+
--
[,bash]
----
# 安装 whizard-telemetry 和 vector
kubectl apply -f kse-extensions/whizard-telemetry-installplan.yaml
kubectl apply -f kse-extensions/vector-installplan.yaml 


# 检查 whizard-telemetry 和 vector 的安装状态，等待状态变为 Installed
