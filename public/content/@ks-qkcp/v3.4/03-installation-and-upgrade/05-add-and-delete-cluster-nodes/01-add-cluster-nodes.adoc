...
// :ks_include_id: 324d602d824d4c449259417133f999fc
:ks_xref_id: 6a1616a62d854ce085dc51fd91d3cada
:ks_comp: ../../../components/
:ks_title: 添加集群节点
:ks_keywords: Kubernetes, {ks_product}, 添加集群节点
:ks_description: 介绍如何添加{ks_product_both}集群节点。
:ks_weight: 01
:ks_icon:
...
== 前提条件link:[id=edf4fc7ff15e4963adca3df9339d70ae]

include::../../../_custom/installationAndUpgrade/installationAndUpgrade-prer-obtainInstallationPackage.adoc[]

* 新增节点的操作系统和版本须为 Ubuntu 16.04、Ubuntu 18.04、Debian 9、Debian 10、CentOS 7、RHEL 7、SLES 15 或 openSUSE Leap 15。多个集群节点的操作系统可以不同。关于其它操作系统和版本支持，请联系{ks_product_both}技术支持。
...
* 如果添加控制平面节点，您需要提前为集群配置高可用性。如果您使用负载均衡器，请确保负载均衡器监听所有控制平面节点的 6443 端口。有关更多信息，请参阅link:../../01-preparations/03-configure-high-availability/[配置高可用性]。
...
* 如果您无法获取安装配置文件 **config-sample.yaml**，您需要参阅link:../../02-install-kubesphere/02-install-kubernetes-and-kubesphere/[安装 Kubernetes 和{ks_product_both}]重新创建 **config-sample.yaml** 文件。重新创建该文件时，请务必确保文件中的集群信息与集群的当前实际情况一致。否则，添加节点后集群可能会出现错误。

include::../../../../_ks_components/admonitions/admonEnd.adoc[]
--


== 操作步骤link:[id=3090f51dda84406eb51df5a6c83d0e46]
...
* 如果当前集群使用本地负载均衡实现高可用性，您不需要对集群高可用性进行任何操作；如果当前集群使用负载均衡器实现高可用性，您只需要设置负载均衡器监听所有控制平面节点的 6443 端口。有关更多信息，请参阅link:../../01-preparations/03-configure-high-availability/[配置高可用性]。
