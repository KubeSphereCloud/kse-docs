...
// :ks_include_id: 485c4a045d46495f9812e2bea2325cf8
:ks_xref_id: 247b3a3bebd543849277d370f89f6982
:ks_comp: ../../../components/
:ks_title: 将 SonarQube 集成到流水线
...
link:https://www.sonarqube.org/[SonarQube] 是一种主流的代码质量持续检测工具。您可以将其用于代码库的静态和动态分析。SonarQube 集成到{ks_product_both}流水线后，如果在运行的流水线中检测到问题，您可以直接在仪表板上查看常见代码问题，例如 Bug 和漏洞。

本教程演示如何将 SonarQube 集成到流水线中。在link:../../02-how-to-use/01-pipelines/02-create-a-pipeline-using-jenkinsfile/[使用 Jenkinsfile 创建流水线]之前，请先参考以下步骤。


== 准备工作link:[id=1c0dd5824194486e963d4e80733ed78e]

您需要启用 DevOps 扩展组件。关于更多信息，请参阅《{ks_product_right}平台管理指南》的“扩展组件管理”章节。


== 安装 SonarQube 服务器link:[id=2f970118e67f4234b6026427dadcce13]
...
version.BuildInfo{Version:"v3.5.0", GitCommit:"c4e74854886b2efe3321e185578e6db9be0a6e29", GitTreeState:"clean", GoVersion:"go1.14.11"}
...
== 获取 SonarQube 控制台地址link:[id=2a381c4a7d064ff5884a7ff1522d4aea]
...
. 您可以获得如下输出（本示例中端口号为 **31377**，可能与您的端口号不同）：
+
--
// Bash
include::../../../../_ks_components/code/bash.adoc[]
...
== 配置 SonarQube 服务器link:[id=cae7c461ce5048cf901e00a257116af7]
...
devops-jenkins-68b8949bb-7zwg41/1Running084m
s2ioperator-01/1Running184m
sonarqube-postgresql-01/1Running05m31s
sonarqube-sonarqube-bb595d88b-975941/1Running25m31s
...
. 请使用地址 link:http://NodeIP:30180[] 访问 Jenkins。安装{ks_product_both}时，默认情况下也会安装 Jenkins 仪表板。此外，Jenkins 还配置有{ks_product_left} LDAP，这意味着您可以直接使用{ks_product_both}帐户登录 Jenkins。有关配置 Jenkins 的更多信息，请参阅 link:../../02-how-to-use/01-pipelines/06-jenkins-setting/[Jenkins 系统设置]。
+
--
// Note
include::../../../../_ks_components/admonitions/note.adoc[]
...
您需要指定 **sonarqubeURL**，以便可以直接从{ks_product_both}控制台访问 SonarQube。
...
kubectl editcm -n kubesphere-systemks-console-config
...
== 为新项目创建 SonarQube Tokenlink:[id=2e44ffd57c54488e8b3a48d98f989a14]
...
. 分别选择 **Java** 和 **Maven**。复制下图所示绿色框中的序列号，如果要在流水线中使用，则需要在link:../../02-how-to-use/04-devops-settings/01-credential-management/[凭证]中添加此序列号。

== 在{ks_product_both}控制台查看结果link:[id=c10722946f164c0aa34b0119e54d61f7]

您link:../../02-how-to-use/01-pipelines/02-create-a-pipeline-using-jenkinsfile/[使用 Jenkinsfile 创建流水线]或link:../../02-how-to-use/01-pipelines/01-create-a-pipeline-using-graphical-editing-panel/[使用图形编辑面板创建流水线]之后，可以查看代码质量分析的结果。
