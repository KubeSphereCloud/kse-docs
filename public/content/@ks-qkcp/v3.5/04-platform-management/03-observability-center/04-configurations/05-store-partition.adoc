---
title: 通过数据分区/分片机制扩展 Store
weight: 05
---

本节介绍如何通过数据分区/分片机制扩展 Store。
...
link:../04-config-parameters[组件默认内置参数]中的 `store` 项提供了时间分区的默认配置，用户更新该默认配置项后，将在所有未配置 `timeRanges` 的 `Store` CR 实例中生效。