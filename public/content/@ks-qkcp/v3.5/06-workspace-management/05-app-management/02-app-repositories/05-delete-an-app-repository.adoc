---
title: "删除应用仓库"
keywords: "Kubernetes, {ks_product}, 企业空间, 应用仓库, 删除应用仓库"
description: "介绍如何删除应用仓库。"
weight: 05
---

// :ks_include_id: 0631f8173893458c919669b4936e6403
:ks_xref_id: d2d60e9181f24010a8cd6a5166b777ac
:ks_comp: ../../../../components/
:ks_title: 删除应用仓库
:ks_keywords: Kubernetes, {ks_product}, 企业空间, 应用仓库, 删除应用仓库
:ks_description: 介绍如何删除应用仓库。
:ks_weight: 05
:ks_icon:
:ks_permission: **应用仓库管理**
:ks_navigation: **应用管理 > 应用仓库**


本节介绍如何删除应用仓库。

// Warning
include::../../../../../_ks_components/admonitions/warning.adoc[]

应用仓库删除后，用户将不能从应用仓库安装应用，请谨慎执行此操作。

include::../../../../../_ks_components/admonitions/admonEnd.adoc[]


== 前提条件link:[id=613edd487a67422d85325f8e39fe55b8]

:relfileprefix: ../../../../
* 您需要加入一个企业空间并在企业空间中具有pass:a,q[{ks_permission}]权限。有关更多信息，请参阅xref:06-workspace-management/06-workspace-settings/03-workspace-members/_index.adoc[企业空间成员]、xref:06-workspace-management/06-workspace-settings/04-workspace-roles/_index.adoc[企业空间角色]和xref:06-workspace-management/06-workspace-settings/05-departments/_index.adoc[部门]。

// include::../../../../_custom/workspaceManagement/workspaceManagement-prer-requiredPermission.adoc[]

:relfileprefix: ./


include::../../../../_custom/workspaceManagement/appTemplates/appTemplates-prer-enableAppManagment.adoc[]


== 操作步骤link:[id=adb7e5197f77417b9ef4814e6c6b8cd1]

include::../../../../_custom/workspaceManagement/workspaceManagement-oper-openWorkspacePage.adoc[]

+

include::../../../../../_ks_components/oper-navigate.adoc[]

+

. 在需要删除的应用仓库右侧点击 image:/images/ks-qkcp/zh/icons/more.svg[more,18,18]，然后在下拉列表中选择**删除**。

. 在**删除应用仓库**对话框，输入应用仓库的名称，然后点击**确定**。
