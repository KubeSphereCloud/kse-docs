...
|
安装 EdgeWize 之前，必须先完成 EdgeWize 根证书的导入工作，否则无法安装 EdgeWize。
|===
...
+
--
在**扩展组件安装**页签，点击开始安装前，需修改**扩展组件配置**。
...
+
--
- 安装完成后，默认启用该组件。您可以在扩展中心配置、升级、禁用、卸载 EdgeWize。有关更多信息，请参阅link:https://docs.kubesphere.com.cn/v4.1.1/06-extension-user-guide/03-manage-components/[管理扩展组件]。
...
|
为了更好地使用 EdgeWize，还需要安装并配置 WhizardTelemetry 可观测平台的多个扩展组件以及 KubeSphere 应用商店管理扩展组件，请参阅link:../03-install-extensions[安装依赖扩展组件]。
|===
--

== 步骤 3：修改配置字典

为了使边缘集群的 ks-core 也使用私有镜像仓库，需要修改 host 集群的 extension-edgewize 项目下的 configmap `custom-kubesphere-config`.
...
.  点击**更多操作 > 编辑设置**， 编辑已有的键值对数据，添加 global 配置如下，然后点击**确定**。
+
[,yaml]
----
global:
