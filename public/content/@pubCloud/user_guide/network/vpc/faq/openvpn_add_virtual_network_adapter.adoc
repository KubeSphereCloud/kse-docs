...
用户在安装 OpenVPN 时，默认会创建一块虚拟网卡，若需要同时访问多个 OpenVPN server，会有相应报错 "`All TAP-Windows adapters on this system are currently in use.`"，此时需要创建多块虚拟网卡来访问。
...
. 在适配器中查看创建的网卡 TAP-Windows Adapter V9 #2。
+
