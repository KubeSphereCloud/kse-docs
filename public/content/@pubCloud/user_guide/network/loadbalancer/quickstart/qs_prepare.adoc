...
* *创建一个 VPC 网络*
+
VPC 网络用于部署负载均衡器、后端云服务器。具体操作，请参考link:/user_guide/network/vpc/manual/vpcnet/10_create_vpc/[创建 VPC 网络]。

* *创建一个公网 IP*
+
公网 IP 用于绑定负载均衡器实例。具体操作，请参考link:/user_guide/network/eip/manual/ipv4/outband_ipv4/[创建公网 IP]。
...
+
具体操作，请参考link:/user_guide/compute/vm/quickstart/create_vm/[创建云服务器]和link:../centos_nginx/[部署 Nginx]。
+
