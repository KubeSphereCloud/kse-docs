...
keywords: DNS服务，DNS云解析，DNS API，Record API，解析线路
---
 ...
 ** mode, 记录模式， 取值为：1 普通模式， 2 轮询模式， 3 权重模式， 4 智能模式，可参见：link:../../intro/dns_function/[四种解析模式]
  *** 权重模式仅对 CNAME 和 A 记录有意义
  *** 智能模式仅对 A 记录有意义
  *** 轮询模式，对 A 记录有意义
...
* 返回结果说明：
 .. 返回 group_id 为组 ID，用于按组暂停解析记录。
 .. id 为记录 ID，用于按记录值的唯一标识，将来用于暂停单个解析记录。
 .. 若记录被自动合并，只返回合并记录的值，不会返回全量记录列表。
...
----
+
数据格式说明见 <<_createrecord,Create Record>>。
