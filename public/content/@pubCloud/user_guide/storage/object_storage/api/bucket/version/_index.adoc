...
keywords: 云计算, 青云, QingCloud, 对象存储, QingStor, Bucket
...
此操作要求调用者对 Bucket 拥有可读权限。若指定的版本已删除，则返回 404 或 405。除link:../../common_header/#响应头字段-response-header[公共响应头]外，QingStor 对象存储服务端还会返回如下头字段：
...
用户上传 Object 时，可以使用 link:../../object/basic_opt/put/[PUT Object] 接口。除link:../../common_header/#响应头字段-response-header[公共响应头]外，QingStor 对象存储服务端还会返回如下头字段：
