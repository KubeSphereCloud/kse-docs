...
* 该 API 不支持匿名请求，请先注册青云账号并link:../../signature/#_获取_access_key[创建 Access Key] 后才能调用此 API。
* 该请求可以用于查询 QingStor 对象存储的 Global Endpoint 下所有区域的 Bucket。Global Endpoint 为：`qingstor.com`。
* 该请求也可以用于查询 QingStor 对象存储某个区域的 link:../../../intro/product/#_zone[Zone Endpoint]，如：pek3.qingstor.com 的 Bucket。
...
| Location
| Enum
| 限定 Bucket 所在的区域。目前支持：`pek3a`，`sh1a`。
| 否
|===
