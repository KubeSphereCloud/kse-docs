...
keywords: 云计算, 青云, QingCloud, 对象存储, QingStor, 签名验证
draft: false
weight: 3
---
...
由于移动端应用的特点，将 Access Key 随 APP 分发会带来安全问题。所以 QingStor 对象存储提供了签名服务解决方案，具体请参考 link:../../beat_practices/app_integration/[移动 App 接入方案]。
...
. 登录 https://console.qingcloud.com/login[管理控制台]，选择 *产品与服务* > *访问与授权* > *API 密钥*，进入 API 密钥主页面：
+
