...
* HTML 的表单设置如下： +
`action` 为 `http://<bucket-name>.<zone-id>.qingstor.com`，其中 `zone-id` 可参考 link:../../../intro/product/#_zone[Zone] +
`method` 必须为 `POST` +
...
| charset
| String
| 指定上传的文件名的编码方式。必须与网页开头的 `<meta http-equiv="Content-Type" content="text/html;charset=***">` 中 `charset` 的设置一致。若不指定，QingStor 对象存储默认使用 `UTF-8` 对上传的文件名进行解码。
| 否
