...
---
QingStor 对象存储的 JavaScript SDK 已在 GitHub 开源，本文为简要使用文档。更多详细信息请参见 https://github.com/yunify/qingstor-sdk-js[GitHub 项目]。
...
* 方式三：请前往 https://github.com/yunify/qingstor-sdk-js/releases[Release] 页面下载打包好的 SDK 文件，然后在 HTML 中加上如下格式的 Script 标签引入 SDK，QingStor 对象存储建议用户在开发时使用未压缩的版本，方便调试，在生产环境中使用 `qingstor-sdk.min.js` 版本。
+
[source,html]
----
...
.. 发往 QingStor 对象存储的请求需要使用 Access Key 和 Secret Key 对请求签名，请前往link:https://console.qingcloud.com/access_keys/[ {console_name}]创建和下载。下载到的密钥文件格式如下，请妥善保存你的密钥:
+
[source,plain_text]
----
...
QingStor 对象存储的 SDK 使用 https://github.com/axios/axios[axios] 作为 HTTP 客户端，所有请求的返回都是一个 Promise。axios 的 Response 结构如下:
