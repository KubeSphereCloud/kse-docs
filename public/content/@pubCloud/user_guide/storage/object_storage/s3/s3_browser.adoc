...
. 在弹出的对话框内填写参数。
+
image::/images/cloud_service/storage/object_storage/s3-browser-7.png[s3-browser-7]
+
**页面各字段说明：**

* **Account Name**：任意
* **Account Type**：选择 S3 compatible Storage。
* **Rest Endpoint**：填写 s3.<Zone_id>.qingstor.com，其中 Zone_id 可参考 QingStor 对象存储的区域，目前包括：pek3a/pek3b/sh1a/gd2。
