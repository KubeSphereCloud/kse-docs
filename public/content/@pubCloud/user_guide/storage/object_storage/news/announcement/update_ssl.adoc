...
* Linux 平台
+
. 打开并编辑 `/etc/ca-certificates.conf` 文件；

. 在 `mozilla/DST_Root_CA_X3.crt` 这一行前增加一个英文、半角的感叹号 `!`，以禁用该证书：
+
[source,plain_text]
----
...
. 在 CentOS 7 及其以后的版本，您还需要执行如下命令行：
+
[source,plain_text]
----
