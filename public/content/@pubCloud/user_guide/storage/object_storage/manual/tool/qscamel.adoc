...
QingStor 对象存储服务，能够用做 qscamel 数据迁移任务中的 `source` 与 `destination` 端点，使用 `qingstor` 作为端点时，需在 `options` 字段下添加如下配置内容。
...
| protocol | 访问 QingStor 对象存储的协议类型。可选值 `https`、`http`。
| host |标识访问 QingStor 对象存储的云服务器名。默认值 `qingstor.com`。
| port |标识访问 QingStor 对象存储的端口号。`https` 协议，端口号为 `443`，`http` 协议端口号为 `80`。
| zone |标识访问 QingStor 对象存储 Bucket 的所在区域，无需手动配置。
| bucket_name |待操作的 QingStor 对象存储的 Bucket 名称。
| access_key_id | QingStor 对象存储的 Access Key ID，可参考link:../../../api/signature/#_获取_access_key[前文内容]获取。
| secret_access_key |QingStor 对象存储的 Access Key，可参考link:../../../api/signature/#_获取_access_key[前文内容]获取。
| storage_class |QingStor 对象存储所使用的存储级别。
...
//以下列举示例，均以 QingStor 对象存储为 `destination` 端点。

=== 将本地文件拷贝至 QingStor

`fs` 是指符合 POSIX 标准的文件系统，如 local fs、nfs、s3fs 等。可作为 qscamel 数据迁移任务中的 `source` 端点与 `destination` 端点。

如用户需将本地目录 `/path/to/source` 下的文件 copy 至 QingStor 对象存储的 `example_bucket` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
...
如用户需将本地目录 `/path/to/list` 下的文件列表 copy 至 QingStor 对象存储的 `example_bucket` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
...
如用户需将 S3 的 `example_bucket` 下 `/path/to/source` 目录下的文件 copy 至 QingStor 对象存储的 `bucket_name` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
...
如，将 OSS 的 `example_bucket` 下 `/path/to/source` 目录下的文件 copy 至 QingStor 对象存储的 `bucket_name` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
...
如，将 Azblob 的 `bucket_name` 下 `/path/to/source` 目录下的文件 copy 至 QingStor 对象存储的 `bucket_name` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
...
如，将 COS 的 `example_bucket_url` 下 `/path/to/source` 目录下的文件 copy 至 QingStor 对象存储的 `bucket_name` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
...
如，将 GCS 的 `bucket_name` 下 `/path/to/source` 目录下的文件 copy 至 QingStor 对象存储的 `bucket_name` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
...
如，将 HDFS 的 `example_address` 下 `/path/to/source` 目录下的文件 copy 至 QingStor 对象存储的 `bucket_name` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
...
如，将 Qiniu 的 `example_bucket` 下 `/path/to/source` 目录下的文件 copy 至 QingStor 对象存储的 `bucket_name` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
...
如，将 Upyun 的 `example_bucket` 下 `/path/to/source` 目录下的文件 copy 至 QingStor 对象存储的 `bucket_name` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
...
将 QingStor 对象存储源账户的 `source_bucket` 下 `/path/to/source` 目录下的文件 copy 至 QingStor 对象存储目标账户的 `destination_bucket` 下的 `/path/to/destination` 目录，则可参考以下内容配置任务文件。
