...
对象，是对象存储中储存的基本数据单元，也是用户操作的基本数据单元。每个 Object 的访问地址有两种风格，详情可参考 link:#_endpoint[访问域名]。
...
其中 `zone-id`、`bucket-name`、`object-name` 需根据实际情况进行修改。当前青云开放对象存储的区域及其访问方式如下。

[cols="1,1,1,4", options="header"]
|===
| 区域名 | Zone ID | Region ID | Endpoint

| 北京3区-A| pek3a|pek3a | https://<bucket-name>.pek3a.qingstor.com/<object-name>
| 北京3区-B| pek3b| pek3|  https://<bucket-name>.pek3b.qingstor.com/<object-name>
| 上海1区-A| sh1a| sh1|  https://<bucket-name>.sh1a.qingstor.com/<object-name>
| 广东2区-A| gd2|gd2 |  https://<bucket-name>.gd2.qingstor.com/<object-name>
|===
