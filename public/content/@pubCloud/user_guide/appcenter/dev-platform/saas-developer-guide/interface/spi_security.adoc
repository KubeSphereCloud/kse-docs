...
用户填写的通知 URL 为：`http://www.isvwebsite.com`。

SPI 请求为：

headers: {'app_signature':'qingcloud-app-token'}
url: http://www.isvwebsite.com?p1=1&p2=2&p3=3&signature=qingcloud-app-token

若填写的通知 URL 中本身就含有参数，如：`http://www.isvwebsite.com?platform=qingcloud` ，那么 `platform` 也将参与签名。
