...
. 提交部署后，进入**集群管理**的对应应用的详细信息页面中进行查看。也可在对应的**常用应用**类型中查看已购买的云应用。
+
...
. 找到需要购买的 SaaS 应用，在应用详情中，点击**立即开通** (或 *立即购买*），进入应用开通或应用购买页面。
+
...
. 找到需要购买的 License 应用，在应用详情中，点击**立即购买**，进入**应用购买**页面。
+
...
. 点击**购买应用**，弹出提示窗口。
+
image::/images/cloud_service/appcenter/um_prompt_info.png[]
...
// . 通过以下方式查找到需要购买的 API 应用。

//  ** 在顶部搜索框中输入关键词进行搜索。
//  ** 在左侧应用分类中进行筛选查找。

// . 找到需要购买的 API 应用后，点击应用，进入应用展示页面。
// +
// image::/images/cloud_service/appcenter/api_app_display.png[]

// . 点击**立即购买**，进入 AppCenter 控制台。

// . 在应用购买页面，选择规格与套餐，并阅读和同意 AppCenter 用户协议。
// +
// image::/images/cloud_service/appcenter/bug_api_app.png[]

// . 点击**购买应用**，弹出提示框。

// . 点击**确认**，购买成功，进入**应用管理**页面。
...
// . 通过应用分类或搜索关键字找到需要购买的 API 应用。详见link:../find_app/[查找应用]。

// . 将鼠标移到应用上方，点击**立即购买**，进入**应用购买**页面。
// +
...
// 关于如何查看和使用已购买的 API 应用，请参见link:../mgmt_app/[应用管理]。
