...
. 获取请求中除 signature 外的所有 Query-Params，即 `c=3&b=2&a=1`。
. 将 Query-Params 按照 key 的名称进行字典排序，即 `a=1&b=2&c=3`。
. 使用创建商品时生成的 prod_key 计算签名：base64_encode(HMAC_SHA256(prod_key, “a=1&b=2&c=3”))。
