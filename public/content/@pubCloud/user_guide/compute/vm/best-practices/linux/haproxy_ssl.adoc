...
HAProxy 是一款可实现负载均衡的优秀软件，它可用于 TCP 代理、HTTP 反向代理、SSL 终结、规范 TCP、HTTP 连接等等。本文中主要介绍通过 HAProxy 反向代理实现负载均衡，并确保连接基于 SSL/TLS 安全加密。HAProxy 更多应用详见 link:http://cbonte.github.io/haproxy-dconv/[HAProxy 官方文档]。
...
----
+
image::/images/cloud_service/compute/vm/bestp_linux_haproxy_ssl_1.png[]

.. 找到 `haproxy` 命令源文件，执行如下命令：
+
[,bash]
----
...
`TAGET` 值为内核版本，使用 `uname -r` 查看，2.6 以上的都用 linux26。

include::../../../../../../_components/table_footer.adoc[]


. 下载证书，并上传到云服务器，将证书私钥合并处理。
+
[,bash]
----
...
global
    ...
    tune.ssl.default-dh-param 2048     ## 修改默认使用 2048bit 加密，不设置会有警告
...
frontend  weblb
    ...
    redirect scheme https if !{ ssl_fc }           ## 将 http 请求跳转到 https
    acl is_http hdr_beg(host) www.linuxcamp.club   ## 添加 acl 规则
...
# maxconn 最大连接数 check inter 检测后端可用性频率:2s rise 2 2 次正确可用 fall 3 三次失败不可用
----

=== 方式二：HAProxy 代理 SSL 请求

. 后端绑定好 SSL 证书，可参考 link:../confighttpdssl/[Apache 配置 SSL]、link:../nginx_ssl/[Nginx 配置 SSL]。
+
[,bash]
----
