...
.. 在浏览器输入 `(您的 ip)/phpmyadmin`，如 `123.123.123.123/phpmyadmin`，进入后新建一个名为 `wordpress` 的数据库。
+
...
.. 在浏览器输入 `(您的 ip)/wordpress`，如 `123.123.123.123/wordpress`，就可以到达如下界面：
+
...
.. 安装完成后，再次访问 `(您的 ip)/wordpress`，如 `123.123.123.123/wordpress`，就可以成功登录到 wordpress 了。
