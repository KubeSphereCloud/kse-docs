...
description: 如何使用 SSH 密钥登录云服务器
draft: false
weight: 110
keywords: 云计算, 青云, QingCloud, 云服务器，SSH
linkkey: compute_instances_ssh_access_faq
---
您需要创建一个 SSH 密钥， 然后加载到你的云服务器上并下载私钥文件（例如 `kp-1234abcd` ），详情请参考 link:../../../../ssh/manual/ssh[SSH 密钥操作指南]。

include::../../../../../../_components/note.adoc[]
进行 SSH 连接之前，请确保在云服务器对应的防火墙下行规则中打开 TCP 22 号端口的访问。
...
. 首先link:http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html[下载 putty 和 puttygen]

. 转换密钥格式

启动 puttygen，点击 `+Conversions -> Import key+`，选中您在青云中创建并下载的 SSH 密钥，文件名形如``kp-1234abcd``。然后点击 `Save private key`，您将得到 PuTTY 格式的私钥，如 kp-1234abcd.ppk
...
. 登录 {console_name}，点击**安全** > **SSH 密钥**里创建 SSH 密钥，将这个密钥加载到云服务器上并下载私钥文件，例如 kp-12345678；
. 在 SecureCRT 上创建一个新连接，protocol 是 SSH2，hostname 是 公网 IP 地址，username 为 root；
