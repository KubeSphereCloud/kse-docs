...
. 登录 {console_name}，在公网 IP 监控中开启实时数据，查看当前一小时实时监控数据是否有异常升高，带宽被占满的情况。
+
...
. 我们按照上面查看网络监控的方法，基本可定位到引起带宽占用高的云服务器，然后控制台 VNC 登录到云服务器排查。
+
...
opeak 表示流量峰值
orates 分别表示过去 2s、10s 和 40s 的平均流量
----
...
lsof -i | grep IP
----
. 消耗流量 IP 为 139.198.x.x  ，则执行命令。
+
[source,shell]
----
lsof -i | grep  139.198.x.x
----
. 根据返回结果如下图，得知服务器带宽主要由 Apache 服务进程消耗。
+
