...
. 检查网络连接。除了 IPV6 和多路传送器协议其他都要勾选（保证网卡属性中**网络客户端**，**网络的文件和打印机共享**都勾选）。
+
image::/images/cloud_service/compute/vm/faq_trouble_windows_install_ad_error2.png[]

. 将以下服务设置为自动开启，选择 *控制面板* > *管理工具* > *服务*。
+
...
. （非必要）若域的服务端和客户端均在云平台上，或多个客户端在云上，可参考link:https://www.heelpbook.net/2019/microsoft-changing-sid-of-cloned-vms/[相关手册]，修改客户端虚机的 sid。



. 检查云服务器是否使用了防火墙，包括云平台安全组，云服务器系统防火墙，链路上经过的防火墙，如果使用了需要开通下面端口。建议打开所有端口，待连通后再关闭其他端口
需要开放的端口，可参考link:https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/dd772723(v=ws.10)[相关手册]。
+	
