...
. 登录 {console_name}，选择 *产品与服务* > *安全服务* > *安全组*，进入域名列表页。确认云服务器绑定的安全组已经放行该端口。（如云服务器是通过 vpc 网络进行访问，则修改 vpc 绑定的安全组，并添加端口转发规则）
...
如云服务器是通过 vpc 网络进行访问，则修改 vpc 绑定的安全组，并添加端口转发规则。
