 ...
 ** 指定协议的关键字：包括 ICMP、IP、ARP、RARP、TCP 和 UDP 等协议类型。
* 关于其他参数说明及用法请参见 http://www.tcpdump.org/manpages/tcpdump.1.html[tcpdump 的 Manpage]。
...
. 安装并打开 Wireshark 工具。
. 依次点击 *捕获* > *选项*。
. 在 *Wireshark 捕获接口* 界面中，根据接口名称或对应的 IP 地址选择需要进行抓包的网卡，然后点击 *开始*。
+
