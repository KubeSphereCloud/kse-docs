---
title: 如何通过 ssh 密钥方式访问云服务器
weight: 30
linkkey: compute_instances_ssh_key_faq
---
== 前提条件

. 登录 {console_name}，选择 *产品与服务* > *计算* > *云服务器*，进入域名列表页。
. 在左侧的 *计算基础服务* 导航栏，选择 *计算* > **SSH 密钥**里面创建一个 SSH 密钥， 然后加载到你的云服务器上并下载私钥文件（例如 `kp-1234abcd` ）。
...
. 转换密钥格式
+
启动 puttygen，点击 *+Conversions* > *Import key*，选中您在云平台中创建并下载的 SSH 密钥，文件名形如 `kp-1234abcd`。然后点击 *Save private key*，您将得到 PuTTY 格式的私钥，如 kp-1234abcd.ppk
