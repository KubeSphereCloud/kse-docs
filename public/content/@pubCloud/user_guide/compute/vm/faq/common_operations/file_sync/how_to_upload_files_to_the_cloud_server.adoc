...
* 云服务器名：云服务器的公网 IP。登录 {console_name}即可查看对应云服务器的公网 IP。
* 端口：默认 22。
* 用户名：云服务器的用户名。
* 密码：购买云服务器设置的密码。
...
[source,shell]
----
scp 本地计算机文件地址 用户名@弹性公网 IP:云服务器文件地址    # 将本地文件 /test.txt 上传至弹性公网 IP 地址为 139.198.x.x 的云服务器对应目录下，命令如下：
...
[source,shell]
----
scp 用户名@弹性公网 IP:云服务器文件地址 本地计算机文件地址     # 将弹性公网 IP 地址为 139.198.x.x 的云服务器文件 /test.txt 下载至本地对应目录下，命令如下：
...
* 名称：随意填写，方便记忆
* 主机：云服务器的公网 IP。登录 {console_name}即可查看对应云服务器的公网 IP。
* 协议：默认 sftp 即可。
* 端口：默认 22。
