...
登录 Windows 云服务器，打开浏览器输入如下网站 URL 地址 `www.speedtest.cn`，点击**测试**，等待测速结束即可获取到公网 IP 的上下行带宽，参考如下：
...
* Ubuntu 云服务器
+
此处以 Ubuntu Server 18.04.5 LTS 64bit 系统为例，由测试结果可看出，一个 5 Mbps 的公网 IP，通过第三方 speedtest 测速结果为：
