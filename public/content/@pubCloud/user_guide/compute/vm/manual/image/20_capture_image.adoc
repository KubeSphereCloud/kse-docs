...
+
image::/images/cloud_service/compute/vm/vm_capture_image.png[]

. 点击**创建备份**，弹出提示信息。
. 若已阅读并了解提示信息的注意事项，点击**继续**，弹出**创建备份**窗口。
+
image::/images/cloud_service/compute/vm/vm_bak_win.png[,60%]

. 输入备份名称，并根据需要勾选是否创建新的备份链。
. 点击**提交**，完成备份操作。

=== 制作新镜像

. 备份创建完成后，选择**存储** > *备份*，进入**备份**页面。
. 鼠标右键点击已创建的备份，弹出菜单窗口。
+
image::/images/cloud_service/compute/vm/vm_new_images.png[]

. 点击**制作成新镜像**，弹出**根据云服务器备份制作镜像**窗口。
+
image::/images/cloud_service/compute/vm/vm_new_images_encrypt.png[,60%]
...
. 镜像制作完成后，选择**计算** > *镜像* > *自有镜像*，进入自有镜像页面。
. 查看根据云服务器备份制作的镜像。
+
image::/images/cloud_service/compute/vm/vm_owner_images.png[]
...
* 挂载到云服务器的硬盘必须是本地盘，云盘不支持通过备份创建镜像。关于本地盘和云盘的详细信息，请参见 link:../../../../../storage/disk/intro/introduction/#_硬盘类型[硬盘类型]。
...
. 云服务器创建新的硬盘（硬盘类型选择为**本地盘**），大小也为 20G （和本地系统盘的大小一致；如果系统盘的大小小于 20G，也需要创建 20G 的硬盘）。
. 初始化新创建的硬盘，并挂载新创建的硬盘到某台云服务器上。
...
. 将 test2.img（假定是 qcow2 格式的镜像），转化为 raw 格式；也可以直接转化成 raw 格式后拷贝到云服务器。
+
`qemu-img convert -f qcow2 test2.img -p -O raw test_raw.img`

. 如果镜像的大小小于新创建的硬盘，使用如下指令修改镜像的大小 （当镜像的大小和新创建的硬盘一致时，忽略此步骤）。 必须保证 raw 格式的镜像大小和硬盘大小一致，之后才能使用 dd 拷贝镜像到新创建的硬盘。
+
...
. 在左侧导航栏中，选择**存储** > *硬盘*，将新创建的硬盘制作备份。
. 选择**存储** > *备份*，将新硬盘的备份制作成新镜像，创建用户自定义镜像。
+
image::/images/cloud_service/compute/vm/capture-image-from-vol-snapshot.png[,60%]


. 创建镜像完成后，选择**计算** > *镜像* > *自有*，可查看创建的镜像。
+
