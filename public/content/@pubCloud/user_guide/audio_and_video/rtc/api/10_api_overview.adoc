...
| signature_method
| 使用的签名方法，默认为 HmacSHA256。

| time_tamp
| 请求时间，格式：2021-10-15T06:44:58Z。 +
...
通过管理控制台创建 API 密钥。如果已创建 access_key_id 和 secret_access_key，可跳过此步骤。

. 登录管理控制台。
. 在右上角账户名称下拉列表中，点击 *API 密钥*，进入 **API 密钥**页面。
+
image::/images/cloud_service/audio_and_video/rtc/um_api_accesskey.png[,50%]

. 点击**创建**，弹出**创建 API 密钥**窗口。
+
image::/images/cloud_service/audio_and_video/rtc/um_api_create_accesskey.png[]

. 输入密钥**名称**和**描述**信息，点击**提交**，弹出**下载 API 密钥的私钥**窗口。

. 点击**下载**，下载私钥。
+
image::/images/cloud_service/audio_and_video/rtc/um_api_private_prompt.png[,50%]
+
