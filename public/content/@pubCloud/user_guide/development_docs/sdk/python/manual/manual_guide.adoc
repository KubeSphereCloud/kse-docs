...
title: "快速入门"
draft: false
keywords: SDK, QingCloud, 青云, 创建云服务器, 云计算
---
使用 qingcloud-sdk 前请先在 link:https://console.qingcloud.com/access_keys/[{console_name}] 申请 access key，申请后便可以自由使用了。

include::../../../../../../_components/note.adoc[]
如果您期望在IAM身份的信任设备上使用 qingcloud-sdk ，也可以不用申请 access key ，而是到 link:https://console.qingcloud.com/iam/[IAM 控制面板]创建基于该信任设备的IAM即可。
...
* 若使用 IAM 身份建立连接，则无需提供 `access key id` 和 `secret access key`。
+ 
[,shell]
----
>>> import qingcloud.iaas
...
APIConnection 中各操作函数的返回值是根据 API 返回的 JSON 数据转换而成的 dict 。 具体返回内容可参见 link:../../../../api/[API 文档] 中对应指令。

include::../../../../../../_components/table_footer.adoc[]

== 创建云服务器

创建一台云服务器至少要提供镜像ID，link:../../../../api/common/instance_type/[Instance Types] 或 cpu/memory:
[,shell]
----
...
. 硬盘加载成功后，需要到加载的云服务器上进一步手动操作，具体可参考link:../../../../../storage/disk/quickstart/init/init_start/[硬盘初始化及分区] 。
