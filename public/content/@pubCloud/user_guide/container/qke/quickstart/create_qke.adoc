...
enableToc: false
weight: 10
keywords: 青云, QingCloud, 云计算, 创建, QKE 集群, 节点配置
---
本小节介绍如何快速创建 QKE 集群。
...
* 创建 VPC 网络，用于部署 QKE 集群。具体操作，请参见link:/user_guide/network/vpc/manual/vpcnet/10_create_vpc/[创建 VPC 网络] 。
+
...
* 通过 https://console.qingcloud.com/access_keys/[API 密钥管理页面] 创建 API 密钥。存储、网络等插件通过 API 密钥与云平台交互并创建资源。
...
. 登录 QingCloud 管理控制台。
. 在控制台顶部的导航菜单中，选择**产品与服务** > *容器服务* > *容器引擎 QKE*。
...
include::../../../../../_components/table_footer.adoc[]
 ...
 .. 填写集群基本信息，包括：名称、描述、版本、配置类型、计费方式和可用区。
+
image::/images/cloud_service/container/qke_old/create-basic-settings.png[]
+
系统预置了如下资源配置类型：
...
image::/images/cloud_service/container/qke_old/create-master-settings.png[]
...
image::/images/cloud_service/container/qke_old/create-worker-settings.png[]

==== *客户端节点设置*

客户端节点提供了 VNC 窗口进行登录，可通过客户端节点登录到集群中其他节点。

image::/images/cloud_service/container/qke_old/create-client-settings.png[]

=== 网络设置

image::/images/cloud_service/container/qke_old/create-network-settings.png[]

. 选择 QKE 集群部署的 VPC 网络及私有网络。
+
不同的 QKE 集群可以重复使用相同的 VPC 网络和私有网络，也可以使用不同的 VPC 网络和私有网络。若您已经创建有 VPC 网络和私有网络，可重复使用，不需要多次创建。
+
若您没有可用的 VPC 网络及私有网络，请根据界面提示，点击**新建 VPC 网络**进行创建。VPC 创建的具体操作请参见link:/user_guide/network/vpc/manual/vpcnet/10_create_vpc/[创建 VPC 网络]。
+

include::../../../../../_components/note.adoc[]
* 选择的 VPC 网络与 QKE 集群应在相同的区域。
...
image::/images/cloud_service/container/qke_old/create-external-service.png[]

* *etcd 服务*：Kubernetes 集群使用 etcd 作为后端存储。
+
建议使用青云提供的 link:/user_guide/middware/etcd/[etcd 服务] 单独部署和管理，以获得更好的可用性和容错性。点击**快捷创建**即可创建 etcd 服务集群。etcd 集群和 QKE 集群需部署在同一 VPC 下。
+
如果您未单独部署 etcd 服务，系统将使用内置在主节点的 etcd。

* *ELK 服务*：QKE 集群的日志组件使用 https://github.com/elastic/elasticsearch[Elasticsearch] 作为后端存储。
+
建议使用青云提供的 https://console.qingcloud.com/apps/app-p6au3oyq[ELK 服务] 单独部署和管理，以获得更好的可用性和可维护性。点击**快捷创建**即可创建 ELK 服务集群。ELK 集群和 QKE 集群需部署在同一 VPC 下。
+
如果您没您未单独部署 ELK 服务，系统将使用内置的 Elasticsearch。

include::../../../../../_components/attention.adoc[]
内置 etcd 会占用 QKE 主节点的资源并无法增删节点；内置 Elasticsearch 会占用 Kubernetes 集群的资源并依赖 kKubernetes 进行管理，在 Kubernetes 集群发生故障期间 Elasticsearch 可能无法正常工作。故生产环境下，建议您独立部署 link:/user_guide/middware/etcd/[etcd 服务] 及  link:/user_guide/bigdata/elk/[ELK 服务]。
...
参数详细说明请参见link:/user_guide/container/qke/manual/mgt_cluster/paras_cfg/#_参数说明[配置集群参数]。
