...
draft: false
weight: 10
keywords: 青云, QingCloud, QKE, Kubernetes, 持久化存储, PVC
---
...
所谓容器的 Volume，其实就是将一个宿主机上的目录，跟一个容器里的目录绑定挂载在一起。在 Kubernets 中，卷的类型有很多，不同类型的卷的功能都不一样，具体可以参照文档：http://docs.kubernetes.org.cn/429.html#i[Kubernetes 中文文档-Kubernetes Volume]。
