...
QKE 预安装了 NFS 客户端程序，为了方便用户对接 NFS 服务端，QKE 预置了 https://github.com/helm/charts/tree/master/stable/nfs-client-provisioner[NFS 动态分配器]，支持动态分配存储卷，分配和回收存储卷过程简便，可对接一个 NFS 服务端。

include::../../../../../_components/note.adoc[]
用户也可以使用 https://kubernetes.io/docs/concepts/storage/volumes/#nfs[Kubernetes 官方方法对接 NFS 服务端]，这是一种静态分配存储卷方法，分配和回收存储卷过程复杂，可对接多个 NFS 服务端。
...
创建和挂载存储卷请参见link:../ks/mount_storage/[挂载云平台存储]。
