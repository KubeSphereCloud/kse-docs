...
QKE 从 `v1.0.1` 版本起集成了 https://github.com/yunify/qingcloud-cloud-controller-manager[QingCloud 负载均衡器插件] ，支持将 KubeSphere 内部的服务和 QingCloud IaaS 的负载均衡器关联起来，通过负载均衡器将服务暴露给集群外部调用。

LB 插件具体用法请参考https://github.com/yunify/qingcloud-cloud-controller-manager/blob/v1.3.4/docs/configure.md[文档]。
...
. 配置服务基本信息：名称及所属项目。
+
image::/images/cloud_service/container/qke_old/ks_service_lb_1.png[]

. 点击**下一步**，进行服务设置。
+
image::/images/cloud_service/container/qke_old/ks_service_lb_2.png[]
+
...
. 点击**下一步**，进行高级设置。
+
image::/images/cloud_service/container/qke_old/ks_service_lb_3.png[]
+
...
+
表示负载均衡器的承载能力类型。与 link:/user_guide/network/loadbalancer/api/loadbanlancer/create_lb/[CreateLoadBalancer] 接口中的 `loadbalancer_typ` 取值范围相同。

. 点击**创建**。
+
配置完成后，您便可以通过公网 IP 访问到集群内部服务。
+
image::/images/cloud_service/container/qke_old/ks_service_lb_4.png[]
