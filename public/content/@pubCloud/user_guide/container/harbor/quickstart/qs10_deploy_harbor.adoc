...
在部署 Harbor App 之前，您需要创建一个 VPC 网络和一个负载均衡器。另外，若您需要使用link:/user_guide/storage/object_storage/intro/product/[对象存储]做后端存储，则还需要创建一个对象存储的 Bucket。

. 创建一个 VPC 网络。
+
创建 VPC 网络请参见link:/user_guide/network/vpc/manual/vpcnet/10_create_vpc/[创建 VPC 网络]。
+
...
+
创建负载均衡器及监听器的详细操作，请参见link:/user_guide/network/loadbalancer/manual/lb/create_lb/[创建负载均衡器]及link:/user_guide/network/loadbalancer/manual/monitor/create_http_monitor/[添加监听器]。
+
...
+
image::/images/cloud_service/container/harbor/qs_10_add_rules.png[,70%]

. 创建 Bucket（仅针对使用对象存储的用户，使用本地存储的用户请略过此步骤）。
+
对象存储桶（Bucket）是用户用于存储对象的容器，所有的对象都必须隶属于某个存储空间。
+
创建对象存储桶（Bucket）的详细操作，请参见link:/user_guide/storage/object_storage/manual/console/bucket_manage/basic_opt/#_创建_bucket[创建 Bucket]。
...
image::/images/cloud_service/container/harbor/harbor-create-vxnet-set_q.png[network-setting,70%]
...
使用 QingStor 对象存储服务来存储镜像文件时，对象存储的相关参数一定要填写正确，对象存储桶及存储根目录在集群创建后不可修改，请谨慎填写。

include::../../../../../_components/table_footer.adoc[]

=== 第5步：用户协议

阅读《{platform_shortname} AppCenter 用户协议》并勾选确认接受该协议。
