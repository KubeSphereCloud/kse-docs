...
* 您可以通过 kubectl 连接托管版和自管版集群。对于自管版集群，您还可以link:../ssh_login_cluster/[通过 SSH 方式登录集群]。
...
* 集群已成功创建并正常运行。
* 已配置 *K8s apiserver EIP*。具体操作请参见link:../../mgt_cluster/paras_cfg/[集群参数配置]。

== 操作步骤

=== 步骤一：安装 kubectl

在本地电脑下载并安装 kubectl，具体安装操作请参见link:https://kubernetes.io/docs/tasks/kubectl/install/?spm=a2c4g.11186623.0.0.18417aa2PRtXQX[安装和设置 kubectl]。
...
. 在**集群信息**页面，点击 *查看 kubeconfig*，然后点击**复制**，复制 kubeconfig 文件中的内容。
+
...
. 将 kubeconfig 内容拷贝到 kubectl 客户端机器的 `$HOME/.kube/config` 文件中并保存退出。
+
