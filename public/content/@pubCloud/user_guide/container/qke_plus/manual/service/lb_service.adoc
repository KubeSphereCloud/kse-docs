...
若未创建服务器证书，请参见link:/user_guide/network/loadbalancer/manual/certificate/create_cert/[创建服务器证书]进行创建。
...
+
表示负载均衡器的规格，其值与 link:/user_guide/network/loadbalancer/api/loadbanlancer/create_lb/[CreateLoadBalancer] 接口中的 `loadbalancer_typ` 取值范围相同。
