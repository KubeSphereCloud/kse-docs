...
weight: 15
draft: false
keywords: QKE, 容器, 应用, 镜像
linkkey: qke_doc_question_deploy_image_faq,qke_doc_deploy_image_faq
---
== 如何配置镜像仓库？

支持在 QKE 控制台的**集群信息** > **环境参数**页面中通过 `registry-mirrors` 参数配置您的镜像仓库服务地址。

具体操作请参见link:../../quickstart/cfg_mirror_repo/[配置镜像仓库]。
...
* 如果使用私有镜像仓库，比如{platform_name_cn} 提供的 link:/user_guide/container/harbor/intro/introduction/[Harbor 镜像仓库]，请确保 QKE 所有节点可以访问到 Harbor 的负载均衡器地址。
+
...
使用{platform_name_cn}硬盘作为存储服务的节点，当节点被删除后，节点上的有状态副本集的容器组可能会无法在其他节点重新创建。
