...
| -
| 表示允许跨域资源共享的域。详细配置说明，请参见 link:https://www.elastic.co/guide/en/elasticsearch/reference/7.10/modules-http.html[Elatsticsearch]。
...
| ES 代理最大连接数
| 0~65535
| 表示 Dashboard 节点代理最大并发连接数，超出的客户端连接请求将排队等待。详细连接数说明，请参见 link:https://cbonte.github.io/haproxy-dconv/1.8/configuration.html#4-maxconn[HAProxy Configuration]。默认值 2000。
...
* 不能以斜线（/）结尾。
* 详细配置说明，请参见 link:https://www.elastic.co/guide/en/kibana/master/settings.html[Configure Kibana]。
