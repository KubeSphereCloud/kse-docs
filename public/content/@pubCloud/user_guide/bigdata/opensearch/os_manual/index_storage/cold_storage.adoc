---
title: "快照与恢复"
description: 本小节主要介绍 OpenSearch 索引快照。
keywords: 搜索分析,索引快照,存到 QingStor,对接 QingStor,存到对象存储
weight: 10
collapsible: false
draft: false
---
OpenSearch 支持创建快照（snapshot）将指定 index 甚至整个集群的数据存储到某快照仓库（repository），并能从远端仓库存储的快照中恢复数据。

https://opensearch.org/docs/latest/opensearch/snapshot-restore/#amazon-s3[repository-s3] 是一款将对象存储桶作为快照仓库的OpenSearch 插件。

本章节主要介绍如何管理 OpenSearch 索引快照仓库与快照，包括创建快照、从快照恢复数据、以及查看和删除快照仓库和快照。
...
| region
| 桶文件夹所在的 Region ID（区域 ID），可参考link:../../../../../storage/object_storage/intro/product/#_endpoint[对象存储相关说明]。
| pek3a
|===
...
OpenSearch 支持将快照恢复到另一个的集群，可实现 OpenSearch 集群之间索引导入与导出。
