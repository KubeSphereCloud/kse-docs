...
有关 QingStor 的更多内容，请参考 link:../../../../../storage/object_storage/[QingStor 对象存储用户指南]。
...
| zone
| QingStor 对象存储可用区，目前开放了 pek3b、pek3c、pek3d、sh1a、gd2a、gd2b 区。
...
----
cd /opt/hadoop
# 从 HBase 客户端本地上传文件到 QingStor 对象存储
