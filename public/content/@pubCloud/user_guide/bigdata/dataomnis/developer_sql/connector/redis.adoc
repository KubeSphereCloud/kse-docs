...
https://github.com/QingCloudAppcenter/Flink-Example/tree/main/connector-demo[下载自定义 Redis Connector 代码]。

== 构建 JAR 程序包

将下载的 Connector 代码构建成一个 JAR 包，例如 https://bigdata-doc.pek3b.qingstor.com/dataomnis/doc/flink-connector-redis-0.8.1.jar[flink_connector_redis.jar] 文件。

== 使用自定义 Redis Connector

. 上传程序包 +
 在资源管理界面，上传已构建的 JAR 程序包（例如 `flink_connector_redis.jar`）。详细操作请参见link:../../../manual/resource/upload[上传程序包]。
. 作业引用程序包 +
