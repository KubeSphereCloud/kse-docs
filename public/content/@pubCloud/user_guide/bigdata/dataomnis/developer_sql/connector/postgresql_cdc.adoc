...
| decoderbufs
| String
| PostgresSQL 逻辑解码插件名，可选值：decoderbufs、wal2json、wal2json_rds、wal2json_streaming、wal2json_rds_streaming 和 pgoutput。

| slot.name
| 否
| flink
| String
| PostgreSQL Replication Slot 名称，必须符合 https://www.postgresql.org/docs/current/static/warm-standby.html#STREAMING-REPLICATION-SLOTS-MANIPULATION[PostgreSQL Replication Slot 命名规则]。

| debezium.*
| 否
| 无
| String
| 传递 Debezium 的属性，如：`'debezium.snapshot.mode' = 'never'`。更多信息请查看 https://debezium.io/documentation/reference/1.2/connectors/postgresql.html#postgresql-connector-properties[Debezium 的 PostgreSQL 连接器属性]。
|===
