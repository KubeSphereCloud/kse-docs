...
请参见 link:../kafka#元数据字段[Kafka 元数据字段]。
...
| 无
| String
| 以逗号分隔的 Kafka brokers 地址列表，格式为 `host:port,host:port,host:port`。
...
与常规 Kafka connector 相比，无需指定 key.fields 参数，通过 PRIMARY KEY 来定义。
