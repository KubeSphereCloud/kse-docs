 ...
 ** Apache Bahir 第三方包 +
  https://github.com/apache/bahir-flink[Apache Bahir] 为 Flink 提供了常见的数据源和数据目的的扩展包。
 ** 自行编程实现 +
  自定义 source、sink，详细请参见 https://ci.apache.org/projects/flink/flink-docs-release-1.13/zh/docs/dev/table/sourcessinks/[Flink API]。  +
  示例：link:../redis[自定义 Redis connector]。
. 构建 Connector 包 +
...
. 上传程序包 +
 在资源管理界面，上传自定义 Connector 的 JAR 程序包。详细操作请参见link:../../../manual/resource/upload[上传程序包]。
. 作业引用程序包 +
