...
| 空
| String
| ElasticSearch 索引名称，支持固定 Index（如：`myIndex`），也支持动态 Index（如：`index-{log_ts\|yyyy-MM-dd}` 或 `\{field_name}` ）。详细请参考link:https://nightlies.apache.org/flink/flink-docs-release-1.12/dev/table/connectors/elasticsearch.html#dynamic-index[动态索引]。
...
| json
| String
| 输出格式。目前仅支持 `json`。详细信息， 请参考 link:https://nightlies.apache.org/flink/flink-docs-release-1.12/dev/table/connectors/formats/json.html[JSON 格式页面]。
|===
