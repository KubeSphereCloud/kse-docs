...
title: "WAN 接入点"
linkTitle: "WAN 接入点"
keywords: 云计算, 青云, QingCloud,SD-WAN, WAN 接入点
description: 本章节介绍什么是 WAN 接入点。
draft: false
weight: 20
linkkey: sdwan_gateways_access_manual[[_创建网关接入点]]
---
...
光盒允许您通过 Internet 快速接入 WAN 网。目前,该设备需要通过申请获得，具体步骤请参考link:../../../light_box/manual/base[光盒申请]。

您收到光盒后，把光盒插入电源，并将 Internet 链路插入光盒 WAN 口，然后登录 WEB 平台，在顶部的导航栏里搜索**光盒**，接着在右边区域点击**创建接入点**, 选择**光盒**类型并填入相应的信息即可。

image::/images/cloud_service/sd_wan/1.0/create_wan_cpe.png[]

include::../../../../../_components/note.adoc[]
当您选择"手动开局"时，需要输入光盒序列号。光盒序列号在光盒背面可以找到, 格式类似: 1c2c997dfb81。

include::../../../../../_components/table_footer.adoc[]

创建成功后, 在**光盒**页面, 可查看光盒的连接状态。

image::/images/cloud_service/sd_wan/1.0/cpe_connection_status.png[]


点击光盒接入点进入详情页，可进一步配置光盒，包括 LAN、WAN、静态路由、防火墙和 Wi-Fi。

image::/images/cloud_service/sd_wan/1.0/describe_wan_cpe.png[]

=== 光盒 LAN 配置

在光盒接入点详情页，点击**管理配置**中的**LAN 配置**，可设置光盒的 LAN 网段、网关地址以及是否启动 DHCP 服务。

image::/images/cloud_service/sd_wan/1.0/lan_config.png[,60%]
...
image::/images/cloud_service/sd_wan/1.0/wan_config.png[,60%]
...
image::/images/cloud_service/sd_wan/1.0/describe_wan_cpe_route.png[]
...
image::/images/cloud_service/sd_wan/1.0/describe_wan_cpe_firewall.png[]
...
image::/images/cloud_service/sd_wan/1.0/describe_wan_cpe_wifi.png[]
...
image::/images/cloud_service/sd_wan/1.0/create_wan_line.png[]

创建好的专线接入点，状态是审核中。当完成审核、专线链路施工后，您才可以进行路由配置。目前支持 BGP 和静态路由两种方式。

include::../../../../../_components/attention.adoc[]
对于BGP和静态路由两种方式, 在配置前请慎重选择, 一旦配置不可切换。
...
image::/images/cloud_service/sd_wan/1.0/config_wan_line_bgp.png[,60%]

用户侧 BGP 的 AS 号可以根据需要来修改，修改后需要点击**应用修改**以生效。

image::/images/cloud_service/sd_wan/1.0/describe_wan_line_bgp.png[]

=== 静态路由

采用静态路由方式，需要指定静态路由。可以根据需要指定一条或多条静态路由。

image::/images/cloud_service/sd_wan/1.0/config_wan_line_route.png[,60%]


配置完后，可以在接入点详情页里修改静态路由，修改后需要点击**应用修改**以生效。

image::/images/cloud_service/sd_wan/1.0/describe_wan_line_route.png[]

== 创建网关接入点

网关接入点用于将云平台服务器接入到 WAN 网。在创建网关接入点之前，需要创建并配置边界路由器，具体如下:

. 登录控制台后，搜索**边界路由器**，在右侧点击**创建**即可创建边界路由器。详细操作可参考**边界路由器操作指南**。
. 在边界路由器详情页, 点击**关联 VPC 私有网络**，将私有网络关联到内网路由器。详细操作可参考**边界路由器操作指南**。
. 在内网路由器详情页，点击**路由设置**，进入到内网路由策略配置页面，设置内网路由策略。详细操作可参考**边界路由器操作指南**。
...
image::/images/cloud_service/sd_wan/1.0/create_vpc_wan_access.png[]

include::../../../../../_components/attention.adoc[]
因内网路由器是双线接入 WAN 网, 网关接入点的最大可用带宽是上图中设定带宽的两倍。
...
image::/images/cloud_service/sd_wan/1.0/elastic.png[,60%]
