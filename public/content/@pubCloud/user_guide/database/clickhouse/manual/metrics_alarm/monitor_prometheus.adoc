...
keywords: Prometheus 监控,Grafana 监控,ClickHouse,数据仓库,
weight: 50
collapsible: false
draft: false
---
...
若安装 Prometheus 和 Grafana 的服务器与 ClickHouse 之间网络不通，可通过link:../../../../../network/border_router/[边界路由器]或 link:../../../../../network/vpc/manual/vpn/vpn_intro[VPN] 等方式打通网络。不建议通过**端口转发**的方式将服务暴露到公网，以免造成 ClickHouse 关键信息暴露等风险。
...
* 详细 Grafana 添加数据源说明，请参见link:https://grafana.com/docs/grafana/latest/datasources/add-a-data-source/[Add a data source]。
* 详细 Prometheus 数据源的选项、变量、查询等说明，请参见link:https://grafana.com/docs/grafana/latest/datasources/prometheus/[Prometheus data source]。

== 导入 ClickHouse 模版

Grafana 的 Dashboard 是一组多属性数据的可视化 UI 仪表盘，可自定义仪表盘配置，接入任何配置的 Grafana 数据源。 +
ClickHouse 支持link:https://grafana.com/grafana/dashboards/13500[Clickhouse internal exporter] 模版样式。 +
Grafana Dashboard 导入模版说明，请参见link:https://grafana.com/docs/grafana/v7.5/dashboards/export-import/[Dashboard Export and import]。

== 查看监控

成功导入模版后，即可在 Grafana 平台相应 Dashboard 页面，查看 ClickHouse 监控。

image::/images/cloud_service/dwh_bi/clickhouse/monitor_prometheus.png[Prometheus 监控]
