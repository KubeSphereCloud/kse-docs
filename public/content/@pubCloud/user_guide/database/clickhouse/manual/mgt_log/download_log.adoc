...
keywords: 下载日志,数据仓库,ClickHouse
weight: 20
collapsible: false
draft: false
---
开启日志服务端后，可通过 HTTP 服务预览和下载 ClickHouse 数据库节点日志。HTTP 服务端口为 `18801` ，访问地址为 <节点 IP>:18801
...
+
image::/images/cloud_service/dwh_bi/clickhouse/log_server_addr.png[日志服务地址]
. <<_预览日志>>。
. 下载日志。
+
打开终端，执行下载命令，参考<<_wget_下载日志>>。
