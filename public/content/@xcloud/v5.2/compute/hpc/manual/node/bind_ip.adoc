...
. 选择**产品与服务** > *计算* > *高性能计算 HPC*，默认**集群**页面。
...
. 点击已创建完成的集群 ID，进入集群详情页面，选择**节点列表**，进入**节点列表**页面，点击待解绑公网 IP 的节点所在行的image:/images/cloud_service/compute/hpc/more_operation.png[more_operation,60%]，选择**解绑公网 IP**。
+
