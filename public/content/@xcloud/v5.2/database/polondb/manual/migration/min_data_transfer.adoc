...
若 PostgreSQL 源数据库与 PolonDB 集群不在同一 VPC 私有网络，建议使用 link:../../../../../network/vpc/manual/vpn/[VPN] 方式打通网络。不建议通过**端口转发**方式打通网络，避免因数据库服务信息暴露，造成数据泄漏风险。
...
. 在 PostgreSQL 源数据库，使用 `pg_dump` 将源数据库中的数据保存到磁盘。
+
[,shell]
----
 $ pg_dump \
