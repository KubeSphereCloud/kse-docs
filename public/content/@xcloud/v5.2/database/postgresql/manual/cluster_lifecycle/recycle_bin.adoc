...
keyword: 恢复集群,集群回收站,PostgreSQL,关系型数据库,数据库
weight: 100
collapsible: false
draft: false
---
...
集群删除后，仅可在**回收站 (Recycle Bin)**中保留2个小时，这期间可以进行恢复操作。超过 2 小时，删除的集群资源将被彻底销毁，不可恢复。
...
+
image::/images/cloud_service/database/postgresql/recycle_cluster.png[恢复集群,50%]
