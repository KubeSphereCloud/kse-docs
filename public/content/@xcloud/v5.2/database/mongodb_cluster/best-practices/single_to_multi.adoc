...
. 在 MongoDB Cluster 节点上，执行 dump 数据导入备份。
+
[,shell]
----
...
. 在 MongoDB Cluster 节点上，导出单实例节点的 oplog 到本地。
+
[,shell]
----
...
. 在 MongoDB Cluster 节点上，恢复历史数据到本地。
+
[,shell]
----
...
. （可选）在 MongoDB Cluster 上，恢复增量数据到本地。
+
...
. （可选）在 MongoDB Cluster 节点上，恢复指定时间的数据到本地。
+
[,shell]
----
