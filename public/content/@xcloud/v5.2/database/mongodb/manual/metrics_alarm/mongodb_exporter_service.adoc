...
集成 mongodb_exporter v0.20.9 版本，支持 Prometheus 标准 Exporter 监控指标。详细监控指标项，请参见 https://github.com/dcu/mongodb_exporter[mongodb_exporter]。
...
* 若安装 Prometheus 和 Grafana 的服务器与 MongoDB 网络不通，可通过 link:../../../../../network/vpc/manual/vpn/[VPN] 方式打通网络。不建议通过**端口转发**的方式将服务暴露到公网，以免造成 MongoDB 关键信息暴露等风险。
...
. 设置**Mongodb Exporter: 是否开启**参数值为``是``。 
+
并设置 MongoDB Exporter 端口，默认为 `9001`。
. 点击**保存**，确认启用服务。
+
image::/images/cloud_service/database/mongodb/enable_exporter.png[启动 MongoDB Exporter服务]
...
image::/images/cloud_service/database/mongodb/mongodb_exporter_grafana.png[Grafana 查看监控]

通过访问 `http://<Node IP>:<MongoDB Exporter Port>/metrics`，可验证 MongoDB Exporter 监控是否配置成功。

image::/images/cloud_service/database/mongodb/mongodb_exporter_verify.png[验证 MongoDB Exporter 监控]
...
. 选择目标集群，点击目标集群 ID，进入集群详情页面。
. 在**配置参数**页签，选择**公共参数**，点击**修改属性**。
. 设置 **Mongodb Exporter: 是否开启**参数值为``否``，确认关闭服务。
. 点击**保存**，关闭服务。
