...
* 密码可由数字、字母或 !@#$%{caret}&*()./; 组成，且最少6位。 +
* 不设置则密码为空。

6.x版本及以后版本，该参数值为 **default** 用户密码。
| *✓*
| *✓*
|===
...
* 密码可由数字、字母、下划线或 !@#$%{caret}&*()./; 组成，最低 6 位。
* 如果不希望有验证则此项留空。
| *✓*
| *✓*
...
| maxclients
| 1~65000
| 表示同一时间允许最大的客户端连接数。默认为0，表示无限制，此时最大的客户端连接数为Redis 进程可以打开的最大文件描述符的数量。
| *✓*
| *✓*
...
* volatile-lru：只从设置失效（expire set）的 Key 中选择最近最少使用的Key 进行删除。
...
| 0~600000
| 表示当有操作耗时超过设定的时间后会被记录下来。单位为毫秒（ms）。0表示禁用这个功能。
| *✓*
| *✓*
...
* 默认值为 `admin`。*可以包含小写字母、数字、半角句点（.）和短横线（-)。
* 修改后不会重启 Redis。
| *✓*
| *✓*
