...
* Redis Standalone 集成 redis_exporter 支持 Prometheus 标准 Exporter 监控指标。详细监控指标项，请参见 https://docs.redis.com/latest/rs/administering/monitoring-metrics/prometheus-metrics-definitions/[Metrics in Prometheus]。
...
若安装 Prometheus 和 Grafana 的服务器与 Redis 网络不通，可通过 link:../../../../../network/vpc/manual/vpn/[VPN] 方式打通网络。不建议通过**端口转发**的方式将服务暴露到公网，以免造成 Redis 关键信息暴露等风险。
...
. 设置**Enable redis-Exporte**参数值为``是``。
+
并设置 Redis Exporter 端口，默认为 `9121`。

. 点击**保存**，确认启用服务。
+
image::/images/cloud_service/database/redis_standalone/enable_exporter.png[启动 Redis Exporter 服务]
...
. 选择目标集群，点击目标集群 ID，进入集群详情页面。
. 在**配置参数**页签，选择**公共参数**，点击**修改属性**。
. 设置**Enable redis-Exporte**参数值为``否``，确认关闭服务。
. 点击**保存**，关闭服务。
