...
* Redis Standalone 集成 node_exporter 支持 Prometheus 标准 Exporter 监控指标。详细监控指标项，请参见 https://github.com/prometheus/node_exporter[node_exporter]。
...
. 选择目标集群，点击目标集群 ID，进入集群详情页面。
. 在**配置参数**页签，选择**公共参数**，点击**修改属性**。
. 设置**Enable Node-Exporte**参数值为``否``，确认关闭服务。
. 点击**保存**，关闭服务。
