...
取值示例 {"database":" * ","passwd":"Passwd@123","ssl":"NO","host":"%","user":"u1","priv":"StandardAccount"} 

* `database` 表示授权访问的数据库名称。可授权访问一个数据库（不支持同时授权多个数据库）或全部数据库（取值 `*` ）；当创建高级权限用户时，可授权全部数据库。 +
* `passwd` 表示数据库帐号密码。密码长度范围 8~32 个字符；必须同时包含大小写字母（a～z，A～Z）、数字（0～9）和特殊字符（@#$%{caret}&*_+-=）。 +
* `ssl` 表示是否开启帐号加密认证。取值 `NO` 则不开启；取值 `YES` 则开启，需同时开启数据 link:../config_ssl_node[SSL 传输加密服务]。 +
* `host` 表示允许访问的主机。取值 `%` 允许所有主机访问。若需指定多个 IP 或网段，可用分号分隔。若需外网访问数据库，需授权全部主机可访问。 +
* `user` 表示数据库帐号名称。不支持添加 `root`、`qc_admin`、`qc_repl` 和 `qc_check` 帐号；只能由大小写字母（a～z，A～Z）、数字（0～9）和下划线（ _ ）组成；长度范围 2~26 个字符。
