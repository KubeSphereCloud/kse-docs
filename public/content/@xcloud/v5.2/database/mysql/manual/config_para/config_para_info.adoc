...
| Load_read_requests _to_master
a| * All 
* All_exclude_master 
* Maininstance_exclude _master 
* Readinstance
a| 表示数据库是否将读请求负载到 Master 节点，默认为 `All_exclude_master`，此时读请求只负载到 Follower 节点。

* ``All``表示读请求负载到所有节点。 
* ``All_exclude_master`` 表示负载到除 Master 外的其他节点。 
* ``Maininstance``表示负载到所有主实例。 
* ``Maininstance_ exclude_master``表示负载到除 Master 外的其他主实例。 
* ``Readinstance`` 表示负载到所有只读实例。

| Not_load_read _to_much _delay_slave
| 0～172800
a| 默认为0，忽略该机制。 

* 若设置大于0，则读 vip 新请求不会分发到延迟秒数大于该值的从节点。 
...
| MSemi-sync-timeout -for-two-nodes
| 1000～10000
a| 表示半同步超时参数，针对主实例数为2的集群有效。 
...
| Audit_log_rotate_ on_size
| 20971520 ~ 2147483648
a| 表示单个审计日志文件大小。 

* 默认为 `100 MB`，范围为 `20 MB` ~ `2 GB`。
* 若该参数不是4096的整数倍，将下调到4096的整数倍。
...
| Expire_logs_days
| 2~14
| 表示 binlog 日志文件保留时间，单位为天。默认为 7天。

| Binlog_cache_size
| 4096~ 3221225472
| 表示 binlog 事务语句日志缓存空间大小。默认为 32768。

| Binlog_stmt _cache_size
| 4096~ 3221225472
...
| Log_bin_trust _function_creators
| 0～1
a| 表示控制是否可以信任存储函数创建者，不会创建写入二进制日志引起不安全事件的存储函数。 只有当二进制日志启用后才会起作用。

* 若设置为0（默认值），用户不得创建或修改存储函数，除非用户同时具有 `CREATE ROUTINE或ALTER ROUTINE` 特权加``SUPER`` 权限）。 设置为 0 还强制使用 `DETERMINISTIC` 特性或 `READS SQL DATA` 或 `NO SQL` 特性声明函数的限制。
* 若设置为1，MySQL不会对创建存储函数实施限制。

| Long_query_time
| 0~300
| 表示慢查询日志时间，单位为秒。默认值为 3s。

| Lower_case_ table_names
| 0～1
a| 表示大小写敏感参数。默认为 0。
...
| Sql_mode
a| * ONLY_FULL _GROUP_BY
* STRICT_TRANS _TABLES 
* NO_ENGINE _SUBSTITUTION
* NO_ZERO _IN_DATE* NO_ZERO_DAT
* ERROR_FOR_DIVISION _BY_ZERO
* PIPES_AS_CONCAT
* IGNORE_SPACE
* NULL
a| 表示 SQL 模式。
...
| Innodb_flush_method
| fsync、O_DIRECT
a| 表示 Innodb数据文件及 redo log 的打开、刷写模式。 

* `fsync`(默认)表示调用fsync()去刷数据文件与 redo log 的 buffer。 
* `O_DIRECT` 表示 InnoDB 使用O_DIRECT 打开数据文件，使用fsync()刷写数据文件跟redo log。
...
| Connection_control _failed_connections _threshold
| 0~ 2147483647
| 表示允许账户连续登录失败的次数。 +
设置为 0，关闭该机制。

| Connection_control _min_connection _delay
| 1000~ 2147483647
| 表示账户连续登录失败次数达到上限后的最小延迟响应时间，单位是毫秒。默认为 1000。

| Connection_control_ max_connection_ delay
| 1000~ 2147483647
| 表示账户连续登录失败次数达到上限后的最大延迟响应时间，单位是毫秒。设置为 2147483647。

| Explicit_defaults_ for_timestamp
a| * 0
* 1
| 表示决定 MySQL 服务端对 timestamp 列值的处理功能。默认为 0，表示关闭。

| Innodb_adaptive_ hash_index
a| * 0
* 1
| 表示是否开启控制 InnoDB 频繁访问的数据缓存在主内存功能。默认为 0，表示关闭。

| Innodb_autoinc_ lock_mode
a| * 0
* 1
* 2
a| 表示控制在向有auto_increment 列的表插入数据时，相关锁的模式。 

* 设置为 0，表示传统锁模式。 +
* 设置为 1，表示InnoDB 使用轻量级别的 mutex 锁来获取自增锁，替代最原始的表级锁。 +
* 默认设置为 2，表示所有情况插入都使用轻量级别的 mutex 锁（只针对row模式）。
...
| Block_encryption _mode
a| * aes-128-ecb* aes-192-ecb
...
| Binlog_transaction _dependency_tracking
a| * WRITESET
* COMMIT_ORDER
...
| Group_replication _transaction_size _limit
| 0~ 2147483647
...
| 5
| 表示高可用组件在连续失败心跳次数达到该值后发起选举。默认为5次，不可修改。
