...
+
image::/images/cloud_service/database/redis_cluster/enable_webconsole.png[开启文件查看]
...
. 在云服务器的浏览器窗口输入访问地址： `http://[Redis IP地址]:80` 。
+
若未设置密码，将直接进入文件查看页面。
+
若设置了密码，将弹出身份验证窗口，需要输入用户名及密码，点击**确定**，然后进入文件查看页面。
+
image::/images/cloud_service/database/redis_cluster/downfiles.png[文件查看器]

. 点击节点 IP，便可对应节点的文件。点击具体文件进行下载。
+
