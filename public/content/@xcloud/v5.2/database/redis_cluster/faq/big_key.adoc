...
* 云服务器中已安装 Redis 客户端。安装步骤可参见 link:../../manual/connect/redis_cli/[redis-cli 连接]中的步骤1。
+
...
----
+
* `<redis_instance_address>`：表示 Redis Cluster 实例的 IP 地址，请根据实际地址替换。
* `<port>`：表示 Redis Cluster 实例的端口号，默认为 6379。
* `<password>`：表示 Redis Cluster 实例的认证密码。
* `--bigkeys`：表示查找大 key。
+
执行成功后，系统显示类似如下信息。
+
