...
| verbose
| Integer
| 是否返回冗长的信息，若为1，则返回应用了此防火墙的其他资源的信息，默认为0.
| No
...
| security_group_set
| Array
| JSON 格式的防火墙数据列表，每项数据可见下面 **Response Item**
...
当你修改防火墙的规则后，要"`更新规则`"才会生效。 更新规则 API 参见 link:../apply_security_group/[_ApplySecurityGroup_]
...
| create_time
| TimeStamp
| 防火墙创建时间，为UTC时间，格式可参见 http://www.w3.org/TR/NOTE-datetime[ISO8601].
|===
