...
QingStor 对象存储建议用户下载使用最新版本的 qsctl，若您仍需查看 qsctl v1.x 版本的相关文档，请点击 link:../qsctl/v1[qsctl v1 文档]。


== 安装

=== 下载安装
. 直接 link:https://releases.qingstor.dev/#qsctl[点击此处]，根据客户端操作系统版本，下载相应安装包。若无特殊要求，QingStor 对象存储建议下载最新版本。
+
...
. 在 link:https://console.qingcloud.com/access_keys/[管理控制台] 申请 API 密钥，并下载保存。
...
. 将本地文件上传至 QingStor 对象存储的 Bucket 中:
+
[source,shell]
----
...
. 将本地文件夹上传至 QingStor 对象存储的 Bucket 中:
+
[source,shell]
----
...
. 将本地文件夹下的所有文件上传至 QingStor 对象存储的 Bucket 中:
+
[source,shell]
----
$ qsctl cp /path/to/folder/ qs://mybucket/parent/ -r
...
. 以列出指定 Bucket `mybucket` 下所有对象的详细信息:
+
[source,shell]
----
...
. 根据提示，进行相关操作即可。在命令行中，QingStor 对象存储新增了对历史命令和自动补全的支持。详细说明如下：
+
* 在行开头可以自动提示补全可用命令；
