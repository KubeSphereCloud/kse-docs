...
用户可在 link:https://github.com/qingstor/qscamel/releases[releases] 页面分别获取适用于 Linux，macOS 以及 Windows 操作系统的最新版本的 qscamel，并进行安装。
...
* `type` 用于定义该任务的类型。可选值: copy，fetch，delete。
...
* `path` 当前端点的路径。须为目录名。
* `options` 标识后续字段为可选字段。不同端点的可选字段，会有区别，详情可参考link:#_端点信息[端点信息]
* `ignore_existing` 用于控制是否跳过已经存在的文件，为空或未配置时将会禁用该配置，即总是覆盖。可选值为：last_modified 与 md5sum。 
...
换句话说，数据迁移任务在创建成功后就不能修改。若用户需修改一个任务内容，请先 link:#_delete[删除] 该任务后，再创建一个新任务，或直接创建一个新任务。创建后的任务，无论执行完成或执行失败，均不会自动从数据库中删除，须人为执行 link:#_delete[删除] 或 link:#_clean[Clean] 操作。

详细操作步骤如下：

. 用户可执行如下命令行，查看 `run` 的使用教程：
+
[source,shell]
----
qscamel run -h
...
. 在任务文件没有更新的前提下，若用户需要恢复一个数据迁移任务，可执行如下命令行：
+
[source,shell]
----
...
. link:#_安装[安装] qscamel 工具。
. 更新 qscamel link:#_配置[配置]。
. 根据如下内容创建任务文件，并保存为 `example-task.yaml`：
+
[source,shell]
----
type: copy
...
- `access_key_id` 与 `secret_access_key` 可参考获取 Access Key 相关内容。
...
Aliyun 是 link:https://www.aliyun.com/product/oss[阿里云] 提供的对象存储服务。可作为 qscamel 数据迁移任务中的 `source` 端点。
...
Azure Blob 是 link:https://azure.microsoft.com/zh-cn/services/storage/[Microsoft] 提供的存储服务。可作为 qscamel 数据迁移任务中的 `source` 端点。
...
Tencent COS 是 link:https://cloud.tencent.com/product/cos[Tencent] 提供的对象存储服务。可作为 qscamel 数据迁移任务中的 `source` 端点。
...
GCS(Google Cloud Storage) 是 https://cloud.google.com/storage/[Google] 提供的对象存储服务。可作为 qscamel 数据迁移任务中的 `source` 端点。
...
hdfs 是 http://hadoop.apache.org/[Hadoop] 的分布式文件系统，可作为 qscamel 数据迁移任务中的 `source` 端点。
...
Qiniu 是 https://www.qiniu.com/[Qiniu] 提供的对象存储服务。可用作 qscamel 数据迁移任务中的 `source` 端点。
...
S3 是 https://amazonaws-china.com/cn/s3[AWS] 提供的对象存储服务。可作为 qscamel 数据迁移任务中的 `source` 和 `destination` 端点。
...
upyun 是 https://www.upyun.com/[Upyun] 提供的对象存储服务。可作为 qscamel 数据迁移任务中的 `source` 端点。
