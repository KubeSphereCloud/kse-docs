...
对象存储的 Bucket 本质上是一个平级结构，但控制台界面会根据 `/` 来模拟文件系统的层级结构。
...
对于大量的未完成分段上传，可通过配置 link:../../manual/console/bucket_manage/lifecycle/[生命周期] 规则自动删除。
...
更详细内容请参考文档 link:../../manual/console/bucket_manage/access_control/#_存储空间策略bucket_policy[设置存储空间策略]。
