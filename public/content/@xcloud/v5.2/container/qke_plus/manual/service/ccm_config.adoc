...
+
https://github.com/yunify/qingcloud-cloud-controller-manager[cloud-controller-manager] 是自主研发的负载均衡器插件，自动集成于 QKE 中。支持将 Kubernetes 内部服务和负载均衡器关联起来，可自动创建负载均衡器、公网 IP，通过负载均衡器将服务暴露给集群外部调用。
...
* 通过在 Service 的 `annotations` 中添加 `ServiceAnnotationLoadBalancerType` 来配置负载均衡器的规格。不添加此 key 的 Service 在 Event 中会有错误。其值范围与 link:../../../../../network/loadbalancer/api/loadbanlancer/create_lb/[CreateLoadBalancer] 接口中的 `loadbalancer_typ` 取值范围相同。
...
监听器参数说明，请参考link:../../../../../network/loadbalancer/api/listener/modify_listener_attribute/[监听器 API 文档]。
