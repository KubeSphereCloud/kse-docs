...
linkkey: containers_qke_lb_service_manual
---
...
. 在下拉列表中选择需要使用的负载均衡器。
+
image::/images/cloud_service/container/qke/service_select_lb.png[70%]
...
若未创建服务器证书，请参见link:../../../../../network/loadbalancer/manual/certificate/create_cert/[创建服务器证书]进行创建。
...
+
表示负载均衡器的规格，其值与 link:../../../../../network/loadbalancer/api/loadbanlancer/create_lb/[CreateLoadBalancer] 接口中的 `loadbalancer_typ` 取值范围相同。
