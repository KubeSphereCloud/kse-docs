...
* 已安装 Docker。具体操作，请参见 https://docs.docker.com/get-docker/[安装Docker]。
+
...
* 已登录镜像仓库。具体操作请参见 link:../../../quickstart/qs18_access_harbor/#docker-登录[Docker 登录]。
...
* `Harbor_address` 表示您的 Harbor 镜像仓库地址，需要与link:../../man08_mdy_para/[参数配置页面]中的 **Harbor 地址**保持一致。
* `ProjectName` 表示项目名称。
* `Repsitory` 表示镜像仓库名称。
* `Tag` 表示镜像标签，通常表示镜像版本。
