...
+
初始私有网络关联默认路由表。操作详情参见link:../../../vpc/manual/vpcnet/10_create_vpc/[创建 VPC 网络]。
+
...
. 在菜单栏，选择**产品与服务** > *网络服务* > *负载均衡器*，进入**负载均衡器**页面。
. 点击**创建**，创建一个负载均衡器。
+
...
+
创建时，**类型**需选择非免费型，*公网 IP* 选择**购买并绑定**，为 VPC 网络绑定公网 IP。详细操作请参见link:../../../vpc/manual/vpcnet/10_create_vpc/[创建 VPC 网络]。
+
