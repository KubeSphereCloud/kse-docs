...
. 自定义一个名字，填写所连接的Kafka集群地址，提供的 Kafka 服务对应的命名空间路径为：zkhost1:port,zkhost2:port.../kafka/集群ID。
+
*示例：*   +
...
. 选择Kafka对应的版本，例如 Kafka 版本为 2.2.0，可以选择 2.2.0，勾选 jmx 配置。
. 更改基本配置，点击 *Save* 后可以使用 Kafka Manager 来管理和监控 Kafka 集群了。
+
image::/images/cloud_service/middware/kafka/add_cluster.png[Add clusters]
