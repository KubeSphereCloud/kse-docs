...
* 可根据镜像ID，状态，镜像名称、操作系统平台作过滤条件，来获取镜像列表。
* 如果不指定任何过滤条件，默认返回你所拥有的所有镜像。
...
| provider
| String
| 镜像提供者。初始青云系统会提供一系列默认镜像，其 provider 为 system 。 当用户捕获{instance_product_name_cn}后，被捕获的"自有"镜像的 provider 为 self 。 +
...
| recommended_type
| String
| 运行该镜像的推荐{instance_product_name_cn}配置
...
* pending： 等待被创建 
* available： 可用状态，此时可以基于该镜像创建{instance_product_name_cn}。 
* deprecated： 已被弃用，此时不再可以基于该镜像创建新的{instance_product_name_cn}，但不影响已有{instance_product_name_cn}的正常使用。 
...
----
https://api.domain.com/iaas/?action=DescribeImages
&COMMON_PARAMS
----
