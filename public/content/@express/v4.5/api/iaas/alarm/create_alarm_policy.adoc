...
| alarm_policy_type
| String
| 告警策略类型: {instance_product_name_cn}（instance），路由器（router），公网 IP（eip），负载均衡器（loadbalancer_listener_http / loadbalancer_listener_https / loadbalancer_listener_tcp / loadbalancer_backend_http / loadbalancer_backend_tcp），集群（cluster_node）
| Yes
...
[,shell]
----
https://api.domain.com/iaas/?action=CreateAlarmPolicy
