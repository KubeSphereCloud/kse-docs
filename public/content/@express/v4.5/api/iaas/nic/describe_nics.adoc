...
可根据mac地址，名称，{instance_product_name_cn}，网络，网络类型，状态，作过滤条件，来获取网卡列表。 如果不指定任何过滤条件，默认返回你的所有网卡。
...
| instance_id
| String
| 挂载的{instance_product_name_cn}id
...
| sequence
| Integer
| 网卡在{instance_product_name_cn}上的序号

| role
| Integer
| 网卡角色 1:主网卡，每个{instance_product_name_cn}一个，是{instance_product_name_cn}绑定eip，防火墙时的默认网卡; 0：从网卡，每个{instance_product_name_cn}可以挂载多个
|===
...
----
https://api.domain.com/iaas/?action=DescribeNics
