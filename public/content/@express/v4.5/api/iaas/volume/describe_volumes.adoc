...
* pending： 等待被创建 
* available： 可用状态，此时硬盘可以加载到{instance_product_name_cn}上。 
* in-use： 使用中，此时硬盘已经加载到{instance_product_name_cn}上。 
...
* attaching： 加载到{instance_product_name_cn}中，由 available 状态变成 in-use 状态 
* detaching： 从{instance_product_name_cn}卸载中，由 in-use 状态变成 available 状态 
...
| instance
| Dict
| 硬盘加载的{instance_product_name_cn}信息. +

* instance_id： {instance_product_name_cn}ID 
* instance_name： {instance_product_name_cn}名称 
* device：硬盘在{instance_product_name_cn}系统中的设备名
...
----
https://api.domain.com/iaas/?action=DescribeVolumes
&status=in-use
&COMMON_PARAMS
----
