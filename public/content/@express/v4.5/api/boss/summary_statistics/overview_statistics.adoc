...
* total_vcpu: vcpu逻辑总量
* used_vcpu: vcpu已分配个数(仅运行中的{instance_product_name_cn})
* used_vcpu_all: vcpu已分配个数(所有状态的{instance_product_name_cn})
* cpu_recommend_rate: cpu的推荐超卖比
---

* real_total_memory: 总内存容量，单位MB
* total_memory: 虚拟内存逻辑总量，单位MB
* used_memory: 虚拟内存已分配量，单位MB(仅运行中的{instance_product_name_cn})
* used_memory_all: 虚拟内存已分配量，单位MB(所有状态的{instance_product_name_cn})
