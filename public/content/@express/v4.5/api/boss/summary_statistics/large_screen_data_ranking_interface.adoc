...
| instances
| 返回{instance_product_name_cn}相关的top统计
a|
...
a|
可通过传入filter以过滤{instance_product_name_cn}的创建者：

* controller=self: 租户自己创建的{instance_product_name_cn}
* controller=pitrix: 系统创建的{instance_product_name_cn}
* controller=all: 不区分是否为租户自己创建的{instance_product_name_cn}
