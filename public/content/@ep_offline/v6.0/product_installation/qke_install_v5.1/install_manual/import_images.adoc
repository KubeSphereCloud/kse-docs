...
如下为{product_name} v5.1 版本推荐的 QKE 版本。
...
. 将下载的 QKE 应用安装包上传到{product_name}的任意一台物理服务器上，记录节点信息和上传的路径。
+
例如，上传到了 demo1r01n02:/pitrix/migrate_appcenter2_qingcloud 目录。
. 登录{product_name}任一 Webservice 节点。
+
include::../../../../../_components/note.adoc[]
可以在{product_name} FirstBox 节点上执行 ``cat /etc/hosts \| grep webservice`` 命令查看 Webservice 节点名称。
...
| -S | 否 | App ID 或 App 版本 ID，若需要导入多个 App 或 App 版本，可用逗号隔开。
| -r | 否 | 请填写环境的 Region ID。
| -z | 否 | 请填写环境的 Zone ID。
