...
* 跨区域复制采用异步复制机制，故数据复制至目标 Bucket 需要一定的时间，通常为几分钟到几个小时不等。
...
| status
| String
a| 该条规则的状态。可忽略大小写，默认值为 `enabled`。 可选值为：
* `enabled` 表示生效。
* `disabled` 表示禁用。
| 否
...
| delete_marker
| String
a| 用于设置是否对删除对象的操作进行复制。默认值为 `disabled`，忽略大小写。可选值为：
* `enabled` 表示启用。
* `disabled` 表示禁用。
| 否
...
| storage_class
| String
a| 目标区域的 Bucket 所使用的存储级别。默认值为 `STANDARD`。可选值为：
* `STANDARD` 标准存储。
* `STANDARD_IA` 低频存储。
| 否
...
| sync_marker
| String
a| 用于设置是否对历史数据进行同步。默认值为 `disabled`，忽略大小写。可选值为：
* `enabled` 表示同步。
* `disabled` 表示不同步。
| 否
|===
