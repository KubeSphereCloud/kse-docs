...
| x-qs-fetch-source
| String
a| 被抓取的文件路径 (源链接)。链接形式为 `<protocol>://<host>[:port]/[path]`。各字段说明如下：

* protocol：可为 `http` 或 `https`，默认为 `http`。
* port： 默认为 `protocol` 对应的端口。
* path：目录。对象存储不会在源链接后边拼接请求中的 `<object-name>`。若 `http` 为空，抓取的结果将取决于该站点默认页的设定。

| 是

| x-qs-fetch-if-unmodified-since
| Date
| 若指定的时间晚于对象存储中的目标 Object 被修改的时间，则从源链接抓取。
| 否
|===
