...
. 在集群详情页面，切换到参数配置页面，选择 Logstash 节点，在 `output_es_content` 配置项后追加如下内容，然后点击保存。
+
[,ruby]
----
...
. 待 Logstash 重启完成后，向 Logstash 发送数据。
+
[,bash]
----
...
. 参照**Kibana 基本用法**在浏览器中访问 Kibana 节点并配置 index pattern，然后点击左侧的 `Discover` 菜单项，显示近期接收到的日志，在搜索栏中输入`中国` ，点击右侧的**搜索**按钮。
+
如图，`中国`被高亮显示并且中间没有空格分隔，测试成功。
+
image::/images/cloud_service/bigdata/elk/search_result.png[search_result]
