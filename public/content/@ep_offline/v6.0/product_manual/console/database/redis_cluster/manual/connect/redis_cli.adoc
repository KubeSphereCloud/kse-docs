...
* 请确保您的云服务器能够连接到公网。
* 本文以 `redis-5.0.8` 版本为例进行介绍，您也可以安装其他版本。具体版本信息，请参见 https://redis.io/download?spm=a2c4g.11186623.2.14.60885347qNKDfG[Redis官网]。
...
. 可选：如果 Redis Cluster 实例设置了访问密码，需要执行以下命令验证密码，校验通过后才可进行缓存数据读写。
+
[source]
----
auth <password>
...
验证成功后，返回 *OK*。
+
image::/images/cloud_service/database/redis_cluster/redis_conet_ok.png[ok]
...
+
image::/images/cloud_service/database/redis_cluster/cluster_nodes.png[nodes]

== 操作步骤 (Windows 系统)

. 下载并安装 Windows 版本的 Redis 客户端。
 .. 登录待安装 redis-cli 的 Windows 云服务器。
 .. 点击 https://github.com/MicrosoftArchive/redis/tags[这里]进行下载。
 .. 解压安装包。
 .. 使用 cmd 工具进入解压目录。
+
[,shell]
...
. 可选：如果Redis Cluster 实例设置了访问密码，需要执行以下命令验证密码，校验通过后才可进行缓存数据读写。
+
[source]
----
auth <password>
