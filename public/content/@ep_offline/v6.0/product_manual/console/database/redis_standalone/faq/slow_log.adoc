...
| slowlog-log-slower-than
 a| 设置慢查询阈值，执行时间超过阈值后会被记录到慢日志。单位为微秒（μs）。
...
参数设置方法请参见link:../../manual/config_para/modify_para/[修改配置参数]。

== 查看慢日志

. link:../../manual/mgt_connect/access_redis_cli/[使用 redis-cli 连接到 Redis 数据库]。
