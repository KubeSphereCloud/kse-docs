...
* 已创建并获取数据库可登录账号和密码，详细说明请参见link:../../mgt_account/creat_account[步骤二：创建数据库账号]。
+
若通过link:../../mgt_connect/enable_external_network[外网地址连接]，需使用**授权主机**配置为 `%` 的账号。
...
通过已获取的link:../../mgt_account/creat_account[数据库用户账号]和连接信息，访问数据库。
...
详细开关 SSL 连接，请参见link:../../ssl_trans_encryption[SSL 传输加密]。
