...
keywords: 云计算,大数据,消息队列,中间件,Etcd,单个节点宕机
weight: 01
collapsible: false
draft: false
aliases: 
...
. 登录异常的 etcd 节点。例如 192.168.0.101 异常。
. 执行以下命令，复制 etcd.service 文件并命名为 etcd1.service。
+
[source]
----
...
. 登录任一正常的 etcd 节点。例如 192.168.0.102。
. 执行以下命令，查看集群节点列表，获取异常节点的 member id。
+
[source]
----
...
. 执行以下命令，复制 etcd.env 并命名为 etcd1.env。
+
[source]
----
...
. 登录之前异常的 etcd 节点。
. 执行以下命令，删除 etcd1.service 和 etcd1.env。
+
[source]
----
...
. 执行以下命令，将 etcd.service 中 EnvironmentFile 的引用恢复为原来的状态。
+
[source]
----
