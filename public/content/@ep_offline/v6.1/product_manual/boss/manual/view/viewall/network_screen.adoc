...
云平台支持跨区域部署，各可用区之间的网络状态关乎平台整体的可用性。为了保障业务正常运行，BOSS 需要监控各 Zone 与 Global 之间的网络连通性与带宽情况。
...
// * 获取三种检查结果：，以及 Zone 与 Global 之间的检查结果。
// * Zone 配置和 Region 配置从上方开始；检查步骤和结果从 hyper 结果 - zone 检查结果 - Region 检查结果，Region 是可选项。
// * Internet 配置步骤从右上角开始；检查步骤和结果从 hyper 结果 - zone Internet - Internet 整体结果，Internet 对于 Zone 和整个云平台都是可选项。
// * Global 配置步骤从右下角开始；检查步骤和结果从 hyper 结果 - zone Global - Global 整体结果。

// image::/images/boss/manual/view/network_screen_1.png[]
