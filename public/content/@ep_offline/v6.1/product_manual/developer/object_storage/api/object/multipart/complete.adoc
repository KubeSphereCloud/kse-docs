...
* 若指定的 Bucket 被设置为匿名用户可写，则请求中可不携带用户认证信息；若请求中仍然携带了用户认证信息，则对象存储仍然会对该用户进行认证，当对象存储认证该用户不拥有该 Bucket 的可写权限，该请求返回错误。
* 该操作允许用户在最终合并分段时，排除掉某些已上传分段。
...
调用该 API 需携带如link:#_示例[请求示例]中的 Json 消息体。该消息体各字段说明如下：
...
Host: mybucket.<zone-id>.test.com
