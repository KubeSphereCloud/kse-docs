...
该接口用于设置 Bucket 的访问策略。对象存储定义只有 Bucket 的所有者才能调用该API。
...
* 每个用户必须为云平台用户，可用云平台用户 ID 表示。
* 总体长度不能超过 300 个字符。
* 该字段不能省略，如果要匹配所有用户，可设置为 `*`。
| 是
...
* 若为 list， 则匹配任意一个就返回 True。

| string_not_like
| Dict
| 字符串非通配，目前仅支持 `*`。

* 如果匹配，返回 False；否则，返回 True。
* 若为 list，则都不匹配才返回 True。
|===
