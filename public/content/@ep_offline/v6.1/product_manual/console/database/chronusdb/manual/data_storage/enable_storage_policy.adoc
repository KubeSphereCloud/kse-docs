...
keywords: 开启对象存储策略；冷热存储，冷热存储策略,时序数据库,ChronusDB,数据库 
weight: 10
collapsible: false
draft: false
---
...
* 若需使用数据冷热分离功能，需link:../config_hot_to_cold_storage[配置冷热存储策略]。
...
对象存储服务与集群需在同一区域；若不在同一区域，可通过link:../../../../../network/border_router/[边界路由器]或 link:../../../../../network/vpc/manual/vpn/vpn_intro[VPN] 等方式打通网络。
...
. 展开下拉菜单，点击**创建对象存储策略**，进入存储策略配置窗口。
+
image::/images/cloud_service/database/chronusdb/enable_bucket_policy.png[启动对象存储策略,50%]

. 配置存储策略信息，详细参数说明请参见<<_存储策略参数,存储策略参数>>。
...
image::/images/cloud_service/database/chronusdb/check_storge_policy.png[查看对象存储策略]
...
| default
| 未创建对象存储策略前，默认存储策略，直接存储数据至热数据盘 `default`。

| ossp
| 开启对象存储策略后，新创建的对象存储策略。数据全量存储到对象存储盘 `ossp`。

| ossp_hot_to_cold
| 开启对象存储策略后，新创建的冷热存储策略。数据可根据磁盘容量和存储时间，在冷热数据盘之间转移，即在 `default` 和 ``ossp``磁盘间转移。
|===
...
* 若需使用冷热存储策略，请参见link:../config_hot_to_cold_storage[配置冷热存储策略]。
