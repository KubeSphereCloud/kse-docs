...
keywords: Zabbix 监控,zabbix server,MongoDB,文档数据库,数据库
weight: 50
collapsible: false
draft: false
---
...
image::/images/cloud_service/database/mongodb/zabbix_arh1.png[zabbix]
...
Zabbix Agent 2 是Zabbix Agent 的升级版，Zabbix Agent 2 降低了与 server 之间的 TCP 连接数，具有更大的检查并发性，易于通过插件进行扩展。Zabbix Agent 2 部分使用 go 语言开发。和 Zabbix Agent 一样支持主动模式和被动模式。两者都可通过配置监控 MongoDB 集群节点，但 Zabbix Agent 和 Zabbix Agent 2 不能同时运行，相关模板也只能使用一个。
...
. 在 *Hosts* 页签，配置 MongoDB 的 zabbix_agent 为监控主机。
 ** *Host name* 自定义主机名称。
...
+
image::/images/cloud_service/database/mongodb/zabbix_create_host1.png[创建 Host]
. 在 *Templates* 页签，选择模版。
+
您可在 *Configuration* > *Templates* 界面自定义模板，详细操作请参见 https://www.zabbix.com/documentation/5.4/zh[Zabbix]。
+
也可选择系统自带且适用于 MongoDB 集群的模板，此处以模板 `MongoDB node by Zabbix agent 2` 为例。
+
image::/images/cloud_service/database/mongodb/zabbix_temp.png[选择模版]

. 在 *Macros* 页签，配置**主机宏**参数。
 ** *{$MONGODB.CONNSTRING}* 配置为 `tcp://localhost:27017`。
 ** *{$MONGODB.USER}* 配置为集群监控服务账户 *qc_monitor 用户*，用户名默认为 `qc_monitor`。
...
+
image::/images/cloud_service/database/mongodb/zabbix_modify_para1.png[配置主机宏]
. 点击 *Add*，创建主机。
+
待主机的 **Status**为 `Enabled` 且 *Availability* 一栏的 `ZBX` 显示为**绿色**后表示监控配置成功，即可查看采集的最新数据和监控图。
+
image::/images/cloud_service/database/mongodb/zabbix_status.png[配置成功状态]
+
...
. 点击主机名称所在行的 *Items*、*Triggers*、*Graphs*、*Discovery*，可查看 Zabbix Server 对 MongoDB 集群支持的监控项、触发器、数据图表等详细监控信息。
+
image::/images/cloud_service/database/mongodb/zabbix_items1.png[查看]
+
image::/images/cloud_service/database/mongodb/zabbix_items.png[查看监控项]
...
可查看监控图表。
+
image::/images/cloud_service/database/mongodb/zabbix_graphs.png[查看]

. 点击右上角时间可筛选对应时间段的监控状况。
+
image::/images/cloud_service/database/mongodb/zabbix_graphs1.png[查看图表]
