...
keywords: 数据迁移,RDB 文件,Redis,Redis Standalone,数据库
weight: 05
draft: false
---
...
* 已link:../../../manual/mgt_files/enable_webconsole[开启 WebConsole 服务]。
* 已获取远端 Redis 数据库信息。
...
若 Redis Standalone 之间网络不通，可通过link:../../../../../network/border_router/[边界路由器]或 link:../../../../../network/vpc/manual/vpn/vpn_intro[VPN] 等方式打通网络。不建议通过**端口转发**的方式将服务暴露到公网，以免造成 Redis Standalone 关键信息暴露等风险。
...
. 在**基本属性**模块，展开下拉菜单。
. 点击**从 RDB 文件恢复数据**，弹出确认窗口。
+
image::/images/cloud_service/database/redis_standalone/rdb_migrate.png[从 RDB 文件恢复数据,50%]

. 阅读提示信息，并配置**确认操作**为``是``。
. 点击**提交**，数据开始恢复。集群状态切换为``更新中``。
. 待状态切换为``活跃``即迁移完成。检查数据，并确认数据迁移完毕。 +
