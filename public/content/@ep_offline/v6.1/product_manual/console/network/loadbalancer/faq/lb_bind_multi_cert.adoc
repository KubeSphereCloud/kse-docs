...
* 已创建 HTTPS/SSL 监听器，具体步骤可参考[添加 HTTPS 监听器]、[添加 SSL 监听器]。
//link:/v6.1/network/loadbalancer/manual/monitor/create_https_monitor/[添加 HTTPS 监听器]、link:/v6.1/network/loadbalancer/manual/monitor/create_https_monitor/[添加 SSL 监听器]。
* 已创建 SSL 证书并完成验证。
