...
. 登录 {platform_name}，选择**产品与服务** > *网络* > *VPC 网络*，进入 VPC 列表页面。
. 找到您的 VPC 网络，点击 VPC 网络名称，进入详情页。
. 切换至**管理配置**页签，然后在左侧点击 *VPN 服务*。
+
image::/images/cloud_service/network/vpc/vpn_service.png[,60%]

. 点击 *OpenVPN* 后面的**打开**，弹出**打开 OpenVPN 服务**对话框。
+
image::/images/cloud_service/network/vpc/webconsole_open_vpn.png[VPN参数配置,45%]
...
推荐使用``证书``或``证书+用户名/密码``认证，以获得更高的安全性。若使用包含``用户名/密码``的验证方式，还需要添加对应的可授权账户列表。
...
例如配置：`push "192.168.10.0  255.255.255.0"`，当客户端向 VPN 服务器请求建立连接成功后，客户端上将添加以上推送的路由配置。
|===

. 可选：如果您需要使用``用户名/密码``的验证方式，则点击**添加账户**，配置账户信息并提交。
...
+
image::/images/cloud_service/network/vpc/openvpn_sg_rule.png[,70%]

== 配置 OpenVPN 客户端

本文主要以``证书``验证方式为例进行客户端配置介绍，若您使用了``用户名/密码``的验证方式，请参见<<用户名密码验证,用户名/密码验证>>进行配置。

启用 OpenVPN 服务后，将出现**下载证书**的按钮，如下图所示。

image::/images/cloud_service/network/vpc/openvpn_config_done.png[,70%]
...
* yunify-ca.crt：{platform_name_cn} CA 根证书
...
. 点击**下载证书 (Windows)**，获取到文件名为 `<vpc_id>-certs.zip` 的 zip 包，然后解压。
. 下载并安装 https://openvpn.net/vpn-client/[OpenVPN客户端（Windows版本）]。
+
...
. 将上述步骤1中下载的 zip 包中的所有文件拷贝到 `C:\Program Files\OpenVPN\config` 目录中。
...
. 启动 OpenVPN 客户端，在任务栏右下角点击 OpenVPN 图标 ，然后点击**连接**建立 VPN 连接。
+
image::/images/cloud_service/network/vpc/win_oneopenvpn_connect.png[,50%]

=== Linux 客户端

以下操作以 Ubuntu 系统为例进行描述，其他 Linux 分发版操作方法类似。

. 点击**下载证书 (Linux)**，获取到文件名为 `<vpc_id>-certs.zip` 的 zip 包，然后解压。
. 安装 OpenVPN。
+
[source]
----
...
. 将上述步骤1下载的 zip 包中的配置文件放入 `/etc/openvpn` 目录中。
...
. 点击**下载证书 (Mac)**，获取到文件名为 `<vpc_id>-certs.zip` 的 zip 包，然后解压。
. 下载并安装 https://tunnelblick.net/downloads.html[Tunnelblick]。
...
+
image::/images/cloud_service/network/vpc/tunnelblick_connect.png[,50%]
...
. 在客户端的配置文件中添加 `auth-user-pass password.txt`， 其中 “password.txt” 是用于保存用户名/密码信息的文件。
+
