...
如下图所示，假设您在{product_name}中有两个 VPC 网络 ，位于同一区域，分别为 router1 和 router2，这两个 VPC 网络各连接有两个私有网络。您可以通过 IPsec 隧道将他们连接在一起。
...
假设本地数据中心的路由器公网地址为 `88.88.88.88`，私有网络为 `192.168.1.0/24`，{product_name} VPC 公网地址为 `99.99.99.99`， VPC 私有网络为 `192.168.100.0/24`，现需要在本地数据中心的路由器中配置 IPsec 隧道实现与 VPC 通信。
...
以下是现阶段{product_name} VPC IPsec 的支持参数，支持参数自动匹配、自动协商。
