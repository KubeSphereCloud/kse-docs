...
---
VPC 网络 IP 是从私有网络 CIDR 分配的一个内网 IP 地址，是{product_name}租户私有的 IP 地址，由租户自己管理，租户可分配、查看和释放地址。
...
* 内网 IP 申请后，不能在 {platform_name}中直接绑定到云服务器，可通过网络配置文件或命令行方式手动添加到云服务器。具体操作请参见link:../04_vip_bind_vm/[为云服务器配置内网 VIP]。
