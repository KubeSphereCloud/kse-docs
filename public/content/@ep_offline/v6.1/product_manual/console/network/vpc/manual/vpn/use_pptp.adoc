...
. 点击 *PPTP* 后面的**打开**，弹出**打开 PPTP 服务**对话框。
+
image::/images/cloud_service/network/vpc/webconsole_open_pptp.png[,50%]
...
. 开启 PPTP 服务后，需要前往 VPC 网络使用的安全组规则中放行 PPTP 服务器运行端口（TCP 下行 1723 端口），同时还需放行 GRE 协议。
+
image::/images/cloud_service/network/vpc/pptp_sg_rule.png[,70%]
...
. 右下角点击网络图标，选择**网络和 Internet 设置**，进入**网络和 Internet** 设置页面。
. 在左侧点击**VPN** > *添加 VPN 连接*。
+
image::/images/cloud_service/network/vpc/win_pptp_1.png[]

. 配置连接参数。
+
image::/images/cloud_service/network/vpc/win_pptp_2.png[]
...
. 点击 VPN 连接名称，然后点击**连接**。
+
image::/images/cloud_service/network/vpc/win_pptp_3.png[]

. VPN 网络连接已经创建后，需要再次连接时，您只需要点击桌面右下角的网络连接处，选择相应的网络连接，然后点击**连接**即可。
+
image::/images/cloud_service/network/vpc/win_pptp_4.png[] 
...
. 右下角点击网络图标，选择**网络和 Internet 设置**，进入**网络和 Internet** 设置页面。
. 在 VPN 设置页面，点击**更改适配器选项**。
. 右键点击对应的 VPN 连接，选择**属性**。
. 选择**网络**页签，双击 *Internet 协议版本 4（TCP/IPv4）*。
. 点击**高级**，取消勾选**在远程网络上使用默认网关**，点击**确定**。
+
image::/images/cloud_service/network/vpc/win_pptp_5.png[]
...
. 修改 `/etc/ppp/chap-secrets` 文件，配置验证所需的用户名和密码，字符串之间用 Tab 键分隔。
+
[source]
----
...
. 新建 `/etc/ppp/peers/<tunnel>` 文件，并配置以下内容。
+
