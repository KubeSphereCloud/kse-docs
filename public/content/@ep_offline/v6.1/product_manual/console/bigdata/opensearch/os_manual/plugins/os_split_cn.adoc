...
keywords: 搜索分析,IK 分词,IK Analysis,自定义分词,中文分词,OpenSearch,搜索引擎
weight: 20
collapsible: false
draft: false
---
...
https://github.com/soosinha/opensearch-analysis-ik[IK 分词插件（OpenSearch）]是基于 Elasticsearch 开源插件 https://github.com/medcl/elasticsearch-analysis-ik[IK Analysis] 开发，集成了 Lucene 的 IK 分析器，支持自定义词典，可用于 OpenSearch 的 IK 分词分析。
...
* 若需通过外网访问 Dashboard，请先通过设置link:../../../../../network/border_router/[边界路由器]或 link:../../../../../network/vpc/manual/vpn/vpn_intro[VPN] 等方式打通网络。
...
. 登录 {platform_name}，选择目标 *OpenSearch 服务*，进入集群详情页面。 +
