...
draft: false
---
本小节为用户呈现了直接调用 Elasticsearch API 的中文分词使用方式，为了在 Elasticsearch 中获取更好的中文分词效果，ELK 集成了 IK Analysis 中文分词插件，并为该插件提供了结巴分词的词库和 IK 自带的搜狗词库，同时还支持用户上传自定义词典。IK Analysis 插件用法请参考 https://github.com/medcl/elasticsearch-analysis-ik[IK Analysis plugin]。
...
. 在集群详情页面中切换到配置参数标签页，选择"Elasticsearch节点"进行参数配置，设置 `remote_ext_dict` 设置项为用户自定义字典的可访问 URL（如示例的 `\http://$LS_IP/dicts/mydict.dic`）后保存, 然后在集群列表页面 *重启* 集群中的 Elasticsearch 节点。
. 执行如下命令测试中文分词功能。
+
[,bash]
----
...
{product_name}的12字决：从初创企业到云生态的蜕变
"
...
. 查看中文分词结果，结果如下图红色部分所示即为成功。即"中国"、"{product_name}"、"优帆科技"被当做固定的中文分词表示。
+
image::/images/cloud_service/bigdata/elk/chinese_split.png[chinese_split]
