...
keywords: Zabbix 监控,zabbix server,ELK,文档数据库,数据库
weight: 10
collapsible: false
draft: false
---
...
image::/images/cloud_service/bigdata/elk/zabbix_arh1.png[zabbix]
...
* 已安装 link:https://www.zabbix.com/cn/download[Zabbix 客户端]，且已获取 Zabbix 系统用户和密码。
+
...
若安装 Zabbix 的服务器与 ELK 网络不通，可通过link:../../../../network/border_router/[边界路由器]或 link:../../../../network/vpc/manual/vpn/vpn_intro[VPN] 等方式打通网络。不建议通过**端口转发**的方式将服务暴露到公网，以免造成 ELK 关键信息暴露等风险。
...
. 在 *Hosts* 页签，配置 ELK 的 zabbix_agent 为监控主机。
 ** *Host name* 自定义主机名称。
...
+
image::/images/cloud_service/bigdata/elk/zabbix_create_host1.png[创建 Host]
. 在 *Templates* 页签，选择模版。
+
...
+
image::/images/cloud_service/bigdata/elk/zabbix_temp1.png[选择模版]
...
include::../../../../../../../_components/table_footer.adoc[]
+
image::/images/cloud_service/bigdata/elk/zabbix_modify_para1.png[配置主机宏]
...
include::../../../../../../../_components/table_footer.adoc[]
+
image::/images/cloud_service/bigdata/elk/zabbix_config_items1.png[]

. 点击页面右上角的 *Create item*，设置 *name*，*Key* 选择 `agent.ping`，其他参数选择默认，点击 *Add*。
+
image::/images/cloud_service/bigdata/elk/zabbix_config_items2.png[]
+
待主机的 **Status**为 `Enabled` 且 *Availability* 一栏的 `ZBX` 显示为**绿色**后表示监控配置成功，即可查看采集的最新数据和监控图。
+
image::/images/cloud_service/bigdata/elk/zabbix_status.png[配置成功状态]
+
...
. 点击主机名称所在行的 *Items*、*Triggers*、*Graphs*、*Discovery*，可查看 Zabbix Server 对 ELK 集群支持的监控项、触发器、数据图表等详细监控信息。
+
image::/images/cloud_service/bigdata/elk/zabbix_items1.png[查看]
+
image::/images/cloud_service/bigdata/elk/zabbix_items.png[查看监控项]
...
可查看监控图表。
+
image::/images/cloud_service/bigdata/elk/zabbix_graphs.png[查看]

. 点击右上角时间可筛选对应时间段的监控状况。
+
image::/images/cloud_service/bigdata/elk/zabbix_graphs_1.png[查看图表]
