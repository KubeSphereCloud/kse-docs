...
keywords: 项目 概述；api 概述
draft: false
weight: 05
---


{product_name}向用户开放所有项目操作相关的 API，API 通过 ``HTTPS GET`` 方式来进行调用。
...
// | 公共参数
// | 每个 API 调用都需要包含公共参数
// | 详情可见link:/v6.1/operation/resource/api/parameters/[公共参数]

// | 指令名称
// | API 指令的名称（action），例如：DescribeInstances 等
// | 完整的指令名称列表请参见link:/v6.1/operation/resource/api/project/create_project/[项目接口]
...
| 返回码
| 返回码（``ret_code``） 用来表示 API 请求的返回值。当 ``ret_code = 0`` 时表示 API 请求正常；``ret_code != 0`` 时表示 API 请求错误
|-
...
include::../../../../../../../_components/note.adoc[]
* *延迟请求*: 对于耗时请求，无法实时返回执行结果，例如 ``RunInstances`` 等。这些请求将以任务形式提交给{product_name}后台执行，并会在 API 请求返回中返回任务 ID（job_id）。这种情况下，用户可以通过查看资源状态来判断任务是否执行完成。例如，在提交 ``RunInstances`` 操作之后，用户可以通过 ``DescribeInstances`` 来查看 Instance 的状态以确认 Instance 是否创建成功。
