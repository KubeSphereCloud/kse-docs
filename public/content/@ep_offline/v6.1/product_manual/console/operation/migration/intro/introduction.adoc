...
通过 V2V 技术将原生或第三方的源系统、源平台等工具或服务，导出为{product_name}所支持的镜像格式，在{product_name}使用导入服务导入，在保证数据一致性的情况下，实现异构平台的数据在{product_name}以云主机的形式运行。
...
|虚拟化平台
|指虚拟软件，用来启动和管理虚机的软件，常见的有：{product_name}，OpenStack，vmware，hyperv xenserver 等。
