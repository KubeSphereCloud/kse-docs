...
. 将资源对应关系通过工单告知{product_name}，需要将老区和新区的资源一一对应，工单标题：[老区迁移]，工单里内容示例：
+
image::/images/cloud_service/compute/migration/manual_migration_1.png[]

+
通过{ platform_name}在新区查看迁移过来的资源，并验证数据完整性。
+
...
利用{product_name}提供的 “资源编排” 功能梳理旧可用区的网络拓扑，为下一步迁移做准备，具体操作如下：
...
. 发工单给{product_name}，告知拓扑ID，如下图中的ID：tp-65v3jzve，工单标题：*老区迁移*
+
工单里内容示例：
+
“请协助将北京2区的资源迁移到北京3区C，资源已经基于资源编排工具创建了，并且所有资源都已经关机，请将北京2区的资源迁移到北京3区C，拓扑ID：tp-65v3jzve。 ”

. 通过{product_name}控制台在新区查看迁移过来的资源，并验证数据完整性。
+
