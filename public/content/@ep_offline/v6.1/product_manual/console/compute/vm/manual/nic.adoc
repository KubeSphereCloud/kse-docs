...
{product_name}网卡是基于虚拟化技术模拟的网卡设备，此设备是基于虚拟机所在的物理设备。{product_name}可以将集群内的虚拟网络统一起来，进行再分配。用户毋需关心具体的网卡在什么位置，有需要申请、挂接即可。


include::../../../../../../../_components/note.adoc[]

每个{product_name}的云服务器最多可以绑定 64 张网卡（其中主网卡 1 张，从网卡 63 张），而每个中网络最多可以有 252 张网卡。
...
. 登录 {platform_name}，选择**产品与服务** > **计算** > **云服务器**，进入云服务器列表页面。
...
建立网卡是为了有效的使用它，那么接下来就将刚才建立的网卡分配到具体的云服务器中。

. 远程连接任一云服务器。如下所示，云服务器操作系统为 Ubunt 16.04，只有一张网卡。
+
[source,shell]
----
root@hosta:~# ifconfig
...
有更多特殊需求，请联系{product_name}技术咨询团队。
