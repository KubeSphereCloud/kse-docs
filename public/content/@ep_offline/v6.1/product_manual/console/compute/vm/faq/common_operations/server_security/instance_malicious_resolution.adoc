...
+
image::/images/cloud_service/compute/vm/faq_instance_malicious_resolution_1.png[]

. 配置 IIS 网站绑定主机名
+
image::/images/cloud_service/compute/vm/faq_instance_malicious_resolution_2.png[]
...
使用IP访问效果
+
image::/images/cloud_service/compute/vm/faq_instance_malicious_resolution_3.png[]

=== Tomcat配置方法

. 修改 `$TOMCAT_HOME/conf` 目录下的 `server.xml` 文件
+
[,shell]
----
...
上面这行的意思是通过 IP 直接访问的是你的默认启动域名所指向的目录。
...
Apache 中对于每个 VirtualHost，都要求有 ServerName 或者 ServerAlias，而且不能为空，具体可以参考 http://httpd.apache.org/docs/1.3/vhosts/name-based.html。

. 进入 Apache 的 `conf` 目录，打开 `httpd.conf` 文件，找到 VirtualHost。
+
[,shell]
----
<VirtualHost *:80>
