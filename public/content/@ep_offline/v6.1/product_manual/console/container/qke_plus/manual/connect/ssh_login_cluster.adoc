...
* *密钥登录*：登录用户为 *root*，登录方式参见 <<通过-ssh-密钥登录,SSH 密钥登录>>。
* *密码登录*：登录用户为 *ubuntu*，登录方式参见<<通过密码登录,密码登录>> 。
...
. 在自管版集群创建页面的**环境配置参数**区域，找到 *K8s apiserver EIP* 参数。
+
image::/images/cloud_service/container/qke/enable_ssh_login_oncreation.png[]

. 选择**绑定已有公网 IP** 或**新建公网 IP**。
. 点击 **SSH 登录集群**后的开关按钮，开启 SSH 登录集群功能。
. 选择登录方式，支持通过 SSH 密钥或密码登录。
...
. 在**集群信息**页面的**环境参数**区域，找到 *K8s apiserver EIP* 参数，点击image:/images/cloud_service/container/qke/edit_icon.png[参数配置]图标，参数值变为可编辑状态。
. 选择一个可用的公网 IP 进行配置。若无可用的公网 IP，点击**新建 K8s apiserver EIP** 进行创建。
+
image::/images/cloud_service/container/qke/enable_ssh_login.png[]
...
. 输入您设置的登录密码，按 *Enter* 即可登录。

== 登录其他节点

. <<登录主节点-master1,登录到集群 master1 节点>>。
. 执行如下命令登录其他节点。
+
*当前为 root 用户：*
+
...
. 可选：ubuntu 用户登录其他节点时，首次登录该节点需要输入密码，再次登录则无需输入密码。
+
