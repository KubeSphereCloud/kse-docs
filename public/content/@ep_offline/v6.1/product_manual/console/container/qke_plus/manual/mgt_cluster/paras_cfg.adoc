...
| 存储插件
| 集群默认存储插件为 csi-qingcloud-controller（{product_name}块存储），您可以添加其他插件提供更多类型集群存储服务，如 csi-u10000（QingStor U10000 存储）。添加前，需要已完成存储与 QKE 的对接，否则无法在界面中添加。
...
| API 密钥 ID
| 此密钥用于创建{product_name}的资源，比如负载均衡器、PV 挂盘等。
|===
