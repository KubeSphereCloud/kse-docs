...
keywords: Harbor, Docker, 镜像, 镜像仓库
weight: 1
---
...
* 已安装 Docker。具体操作，请参见link:https://docs.docker.com/get-docker/[安装 Docker]。
+
...
* 已登录镜像仓库。具体操作请参见 link:../../../quickstart/qs18_access_harbor/#_docker_登录[Docker 登录]。
...
* `Harbor_address` 表示您的 Harbor 镜像仓库地址，需要与link:../../config_para/modify_para/[参数配置页面]中的 **Harbor 地址**保持一致。
* `ProjectName` 表示项目名称。
* `Repsitory` 表示镜像仓库名称。
* `Tag` 表示镜像标签，通常表示镜像版本。
