...
keywords: 云计算,大数据,消息队列,中间件,Kafka,收发消息,常见问题
weight: 30
draft: false
---
...
. 客户端连接 Kafka 服务超时，无法正常收发消息。
. Kafka 集群中有 `不正常` 的 Kafka 节点，且节点的 cpu/mem/ 硬盘使用率过高（如下图，达到了 100%）。
+
image::/images/cloud_service/middware/kafka/abnomal_node.png[]
+
image::/images/cloud_service/middware/kafka/massage_failed01.png[]
...
+
image::/images/cloud_service/middware/kafka/message_failed02.png[]

== 如何增大生产者发送消息的大小？

Kafka 默认生产者发送的消息不能超过 1M（*max.request.size* 参数的默认值为 1M），若需要增大生产者发送消息的大小，需要在生产端和服务端对相关参数进行修改。

. 生产端修改（在 producer 的代码中进行修改）
...
. 服务端修改（通过 {platform_name}的**配置参数**进行修改） +
