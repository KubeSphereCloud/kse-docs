...
本文将详细介绍制作镜像的流程。{product_name} AppCenter 支持三种镜像类型: KVM, Docker。
...
. 云服务器里不要操作资源。如 halt 云服务器等，{product_name}会负责资源调度，用户只需要关注应用即可。
...
. 下载{product_name}提供的 APP Agent 的 link:https://s4.qingcloud.com/files/2022/12/app-agent-linux-amd64.tar.gz[Linux 版本,window=_blank] 或 link:https://s4.qingcloud.com/files/2022/12/app-agent-windows-386.zip[Windows 版本,window=_blank]。
...
* 为了方便开发者存储自己的 docker 镜像，平台提供了 docker 镜像仓库。当前镜像仓库的控制台管理功能尚未完成，所以如果需要使用 docker 镜像仓库，请先提工单申请。为了保证用户安装程序时的体验，应用如果使用 docker 镜像，镜像需要放置到{product_name}的镜像仓库，以保证拉取速度。
