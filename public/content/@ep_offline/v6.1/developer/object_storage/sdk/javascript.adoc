...
---
对象存储的 JavaScript SDK 已在 GitHub 开源，本文为简要使用文档。更多详细信息请参见 https://github.com/yunify/qingstor-sdk-js[GitHub 项目]。
...
* 方式三：请前往 https://github.com/yunify/qingstor-sdk-js/releases[Release] 页面下载打包好的 SDK 文件，然后在 HTML 中加上如下格式的 Script 标签引入 SDK，对象存储建议用户在开发时使用未压缩的版本，方便调试，在生产环境中使用 `qingstor-sdk.min.js` 版本。
+
[source,html]
----
...
具体配置，可参考link:https://github.com/qingstor/qingstor-sdk-js/blob/master/docs/advanced_configuration-zh-cn.md[ Github 上的配置说明]，其中 Access Key，需根据云平台的 Console 上申请的 API 密钥进行填写。

除以上配置外，SDK 使用 ES6 的语法编写，所以使用前，还需确保配置了合适的构建环境。
...
.. 发往对象存储的请求需要使用 Access Key 和 Secret Key 对请求签名，请前往 Console 创建和下载。下载到的密钥文件格式如下，请妥善保存您的密钥。
+
[source,plain_text]
----
...
对象存储的 SDK 使用 https://github.com/axios/axios[axios] 作为 HTTP 客户端，所有请求的返回都是一个 Promise。axios 的 Response 结构如下:
