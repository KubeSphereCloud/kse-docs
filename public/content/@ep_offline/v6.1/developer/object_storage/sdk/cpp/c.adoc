...
在使用 SDK 之前，我们需要创建一个本地的配置文件。也可以在初始化 SDK 时通过调用 `load_config_from_filepath()` 方法来指定具体的配置文件路径。具体配置，可参考link:https://github.com/qingstor/qingstor-sdk-cpp/blob/master/docs/configuration.md[ Github 上的配置说明]，其中 Access Key，需根据云平台的 Console 上申请的 API 密钥进行填写。
...
. 调用 SDK 接口前，需要先指定访问的 Bucket 和 Zone 信息，创建访问句柄。对象存储提供两种创建访问句柄的方式:
...
. 获取存储空间访问控制
+
对象存储支持 Bucket ACL，是 Bucket 级别的访问权限控制。用户可将 Bucket 的读、写、或读写权限开放给单个或多个云平台用户。下面我们将演示如何通过 API 接口来获取和设置 Bucket ACL。
+
[source,c]
----
...
所有的 API 调用接口均与上面的示例相似，用户可以查看 link:../../../api/overview/[API 文档]来了解更多信息。
