...
这里介绍 API 请求中签名的生成方法。您可以根据实际情况选择使用 API 密钥或 IAM 身份来完成签名。

== API 密钥签名

在调用这些 API 之前，您需要先在{product_name}的 {platform_name} 申请 API 密钥，获得 `accesss_key_id` 和 `secret_access_key`。

我们以以下内容为例，加以说明签名步骤。

. 在{product_name}的 {platform_name} 申请 API 密钥内容如下所示。
+
[,shell]
----
...
. 对参数名称和参数值进行 URL 编码，编码后的请求串如下所示。
+
[,shell]
----
{
...
. 构造 URL 请求。参数名和参数值之间用 `=` 号连接，参数和参数之间用 `＆` 号连接，构造后的 URL 请求如下所示。
+
[,shell]
----
...
. 构造被签名串。被签名串的构造规则为：被签名串 = `HTTP 请求方式` + `\n` + `URI` + `\n` + `URL 请求`
+
include::../../../../_components/attention.adoc[]
...
include::../../../../_components/table_footer.adoc[]
+
假设 HTTP 请求方法为 `GET`，请求的 URI 路径为 `/iaas/`，则被签名串如下所示。
+
[,shell]
----
GET\n/iaas/\naccess_key_id=QYACCESSKEYIDEXAMPLE&action=RunInstances&count=1&image_id=centos64x86a&instance_name=demo&instance_type=small_b&login_mode=passwd&login_passwd=QingCloud20130712&signature_method=HmacSHA256&signature_version=1&time_stamp=2013-08-27T14%3A30%3A10Z&version=1&vxnets.1=vxnet-0&zone=pek3a
...
. 计算签名。计算被签名串的签名 Signature。这里以 Python v2.7 的代码为例，其他语言类似。
+
[,python]
----
import base64
...
.. 将 API 密钥的私钥 `ecret_access_key` 作为 `key`，生成被签名串的 HMAC-SHA256 或者 HMAC-SHA1 签名，更多信息可参见 link:http://www.ietf.org/rfc/rfc2104.txt[RFC2104]。
.. 将签名进行 Base64 编码。
...
. 添加签名。将签名参数附在原有请求串的最后面。最终的 HTTP 请求串如下。为了查看方便，我们人为地将参数之间使用回车进行分隔。
+
[,shell]
----
access_key_id=QYACCESSKEYIDEXAMPLE
...
----
+
完整的请求 URL 如下。为了查看方便，我们人为地将参数之间使用回车进行分隔。
+
[,url]
----
...
. 添加签名
+
将身份临时凭证中的 `access_key` 和 `secret_key` 按照前文 API 密钥同样的方法进行签名计算，然后将请求的 URI 路径改为 `/iam/`，最后在请求中附上 `token` 参数，其值为临时凭证中的 `id_token`。
+
以 DescribeInstances 为例，最后请求的 URL 如下所示。为了查看方便，我们人为地将参数之间使用回车进行分隔。
+
[,shell]
----
