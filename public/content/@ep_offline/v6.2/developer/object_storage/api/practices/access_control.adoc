...
weight: 1
---
对象存储通过请求中的link:../signature[签名验证]信息来验证用户身份。签名验证中的 Access Key，需要用户事先从云平台的 Console 申请，并配置到应用程序中。没有经过签名的请求对象存储称之为匿名用户。本章主要介绍经过对象存储验证用户身份之后如何进行权限的控制。
...
Bucket Policy 作为一种数据访问权限设置方式，其对对象存储资源的访问权限的控制粒度是介于 Bucket ACL 和 Object ACL (尚未推出) 之间的。Bucket Policy 可以细致地指定 API 级别的控制，从而实现 Bucket ACL 和 Object ACL 所不能实现的一些功能，如：防盗链。
...
Bucket ACL 对于对象存储的资源访问权限的控制粒度比起 Bucket Policy 和 Object ACL 要更大一些，使用起来也更加简单。其所授予被授权者的权限列表仅可为 READ (可读)，WRITE (可写)，或 FULL_CONTROL (可读写)。
