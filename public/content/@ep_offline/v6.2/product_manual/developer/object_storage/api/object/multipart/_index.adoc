...
1. 客户端调用 link:initiate/[Initiate Multipart Upload] 来初始化一个 Object 分段上传。对象存储返回该 Object 分段上传的唯一 Upload Id。
...
大于 5G 的 Object，对象存储不支持直接拷贝。客户端需要为目标位置 Object 调用 Initiate Upload，然后参照 Copy Object Part 并附带 `x-qs-copy-range` 请求头逐段拷贝数据，最后使用 Complete Multipart Upload 合并得到目标位置 Object。
...
目前 Console 上传大文件是采用 64MB 来作为分段大小。
