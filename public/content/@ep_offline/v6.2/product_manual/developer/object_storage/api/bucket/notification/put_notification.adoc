...
| event_types
| Array
a| 事件的类型，当该类型的事件发生时，对应的操作被触发。可设定的值如下：
...
* tupu-porn: 图谱鉴黄服务。
* notifier: 通知服务，将事件推送到 notify_url。
* image: 图片基本处理服务。
| 是

| notify_url
| String
| 接受通知事件处理结果的 URL。当事件处理完成后，其结果将会以 POST 方式向该 URL 发送。若 POST 超时，对象存储将会重试。目前超时时间为 5s，重试间隔为 1s。
| 否
|===
...
| key_prefix
| String
| 处理结果存回对象存储的 Bucket 的对象名称前缀，默认为 `gen`。
| 否
...
| save_bucket
| String
| 处理结果存回对象存储的目前 Bucket 名称，默认为当前对象所在的 Bucket。
| 否
|===
