...
云监控 CloudSat 可对 MongoDB Cluster 的运行状态进行日常监控。您可以通过 CloudSat  {platform_name}，一站式监控和告警 MongoDB Cluster 各类服务、资源指标。您可以在 MongoDB Cluster 集群详情页面，查看主实例监控和查看备实例监控。

include::../../../../../../../../_components/note.adoc[]

由于云监控 CloudSat 默认监控扫描周期为 5分钟，则当前显示 5～10分钟前的集群状态。
...
* 已创建 MongoDB Cluster 集群，集群状态为``活跃``，且服务状态为``正常``。
+
