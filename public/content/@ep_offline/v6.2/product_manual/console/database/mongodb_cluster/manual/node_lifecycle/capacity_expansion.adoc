...
draft: false
---
MongoDB Cluster 支持对运行中的数据库服务进行在线扩容，调整 CPU、内存、磁盘空间大小。

* 在 AppCenter 集群管理中心，支持对集群节点批量升级扩容。
* 通过配置集群自动伸缩，支持对集群节点自动扩容磁盘大小。


本小节主要介绍如何在线扩容 MongoDB Cluster 集群，包括创建扩容和自动扩容。
...
. 展开下拉菜单，点击**扩容集群**。
+
image::/images/cloud_service/database/mongodb_cluster/expansion.png[创建扩容,50%]
...
3. 点击**创建**，弹出策略基本信息配置窗口。
   ...
   <img src="/images/cloud_service/database/mongodb_cluster/auto_expansion.png" alt="自动伸缩策略" style="zoom:50%;" />
...
5. 确认配置信息无误后，点击**提交**，进入策略详情页面。
    ...
    <img src="/images/cloud_service/database/mongodb_cluster/auto_expansion_rules.png" alt="自动伸缩规则" style="zoom:100%;" />
