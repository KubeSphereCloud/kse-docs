...
* **开启数据迁移**之后，需要执行link:../end_transfer/[结束数据迁移]，才能够进行下一次数据迁移。
...
. 展开下拉菜单，点击**开启数据迁移**。
+
image::/images/cloud_service/database/chronusdb/mir_1.png[开启数据迁移]

. 在**运行开启数据迁移服务**弹出框填写参数。
+
image::/images/cloud_service/database/chronusdb/mir_2.png[开启数据迁移]
+
[cols='1,4']
|===
| 参数 | 说明
...
* YES：增量迁移，定期增量迁移源集群数据。ChronusDB 集群将与源数据库建立主从关系，实时同步迁移源数据库增量数据。
 ...
 ** full：正在进行全量迁移。
+
image::/images/cloud_service/database/chronusdb/mir_full.png[full]

 ** Increment：设置了增量迁移且当前迁移已完成，等待下一次增量迁移。
+
image::/images/cloud_service/database/chronusdb/mir_increment.png[increment]

 ** Incrementing：正在进行增量迁移。
+
image::/images/cloud_service/database/chronusdb/mir_incrementing.png[incrementing]
