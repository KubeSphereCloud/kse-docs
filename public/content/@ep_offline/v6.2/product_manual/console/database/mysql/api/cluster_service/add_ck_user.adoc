...
取值示例 {"passwd":"Passwd@123","host":"::/0","user":"test"} +
* `passwd` 表示 ClickHouse 帐号密码。密码长度范围 8~32 个字符，必须同时包含大小写字母（a～z，A～Z）、数字（0～9）和特殊字符（@#$%{caret}&*_+-=）。 +
* `host` 表示允许访问的主机。取值 `::/0` 允许所有主机访问。若需指定多个 IP 或网段，可用分号分隔。 +
* `user` 表示 ClickHouse 帐号名称。不支持添加 `default` 帐号；只能由大小写字母（a～z，A～Z）、数字（0～9）和下划线（ _ ）组成；长度范围 2~26 个字符。
|===
