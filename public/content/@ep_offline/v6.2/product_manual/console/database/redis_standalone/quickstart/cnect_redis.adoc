...
. 在**节点实时角色**页签，获取 `master` 节点的 IP 地址。
+
image::/images/cloud_service/database/redis_standalone/node_role.png[角色详情]

. 在**服务端口信息**模块，获取 Redis 服务端口或 TLS 端口。
+
image::/images/cloud_service/database/redis_standalone/check_access_info.png[连接信息,30%]

== 步骤 2：访问 Redis

. 参考link:../../manual/mgt_connect/access_redis_cli[通过 redis-cli 方式连接 Redis]，编译 Redis 源码，并安装 redis-cli 工具。
. 进入 `src` 目录，执行访问命令或操作命令。
+
[,shell]
----
...
TLS 加密说明，请参见 link:../../manual/data_security/tls_config[TLS 加密]。
