...
keywords: 数据迁移,redis-port,Redis,Redis Standalone,数据库
weight: 15
draft: false
---
https://github.com/CodisLabs/redis-port[redis-port] 是一款开源的数据批量传输工具，主要用于 Redis 节点间的数据库同步。
...
若安装 redis-port 的服务器与 Redis 之间网络不通，可通过link:../../../../../network/border_router/[边界路由器]或 link:../../../../../network/vpc/manual/vpn/vpn_intro[VPN] 等方式打通网络。不建议通过**端口转发**的方式将服务暴露到公网，以免造成 Redis 关键信息暴露等风险。
...
下载并解压 https://github.com/CodisLabs/redis-port/releases[redis_port] 工具，可直接使用，无需编译。
