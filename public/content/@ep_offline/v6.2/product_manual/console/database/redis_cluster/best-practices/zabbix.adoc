...
keywords: Zabbix 监控,zabbix server,Redis Cluster,文档数据库,数据库
weight: 20
collapsible: false
draft: false
---
...
image::/images/cloud_service/database/redis_cluster/zabbix_arh1.png[zabbix]
...
Zabbix Agent 2 是Zabbix Agent 的升级版，Zabbix Agent 2 降低了与 server 之间的 TCP 连接数，具有更大的检查并发性，易于通过插件进行扩展。Zabbix Agent 2 部分使用 go 语言开发。和 Zabbix Agent 一样支持主动模式和被动模式。两者都可通过配置监控 Redis Cluster 集群节点，但 Zabbix Agent 和 Zabbix Agent 2 不能同时运行，相关模板也只能使用一个。
...
若安装 Zabbix 的服务器与 Redis Cluster 网络不通，可通过link:../../../../network/border_router/[边界路由器]或 link:../../../../network/vpc/manual/vpn/vpn_intro/[VPN] 等方式打通网络。不建议通过**端口转发**的方式将服务暴露到公网，以免造成 Redis Cluster 关键信息暴露等风险。
...
. 登录 {platform_name}。
. 选择**产品与服务** > *数据库与缓存* > *键值数据库 Redis Cluster*，进入集群管理页面。
...
. 在 *Hosts* 页签，配置 Redis Cluster 的 zabbix_agent 为监控主机。
 ** *Host name* 自定义主机名称。
...
+
image::/images/cloud_service/database/redis_cluster/zabbix_create_host1.png[创建 Host]
. 在 *Templates* 页签，选择模版。
+
...
+
image::/images/cloud_service/database/redis_cluster/zabbix_temp.png[选择模版]
...
include::../../../../../../../_components/table_footer.adoc[]
+
image::/images/cloud_service/database/redis_cluster/zabbix_modify_para1.png[配置主机宏]

. 点击 *Add*，创建主机。
+
待主机的 **Status**为 `Enabled` 且 *Availability* 一栏的 `ZBX` 显示为**绿色**后表示监控配置成功，即可查看采集的最新数据和监控图。
+
image::/images/cloud_service/database/redis_cluster/zabbix_status.png[配置成功状态]
+
...
. 点击主机名称所在行的 *Items*、*Triggers*、*Graphs*、*Discovery*，可查看 Zabbix Server 对 Redis Cluster 集群支持的监控项、触发器、数据图表等详细监控信息。
+
image::/images/cloud_service/database/redis_cluster/zabbix_items1.png[查看]
+
image::/images/cloud_service/database/redis_cluster/zabbix_items.png[查看监控项]
...
可查看监控图表。
+
image::/images/cloud_service/database/redis_cluster/zabbix_graphs.png[查看]

. 点击右上角时间可筛选对应时间段的监控状况。
+
image::/images/cloud_service/database/redis_cluster/zabbix_graphs1.png[查看图表]
