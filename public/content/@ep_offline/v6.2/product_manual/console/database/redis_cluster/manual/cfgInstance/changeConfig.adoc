...
* Redis 6.x 更改配置时，只能升级配置，不可降低配置，具体表现为：
 ** 不支持从多线程变更为单线程；
 ** 不支持从生产环境变更为测试环境；
 ** 不支持降低内存。

+
不同配置的规格性能，请参考link:../../../intro/instance_type/[资源配置类型]。
...
. 在集群列表，右键单击目标集群，选择**扩容集群**。
+
以 Redis 6.2.5 版本为例，弹出如下界面：
+
image::/images/cloud_service/database/redis_cluster/mdy_conf_6.0.png[更改配置,50%]
...
+
image::/images/cloud_service/database/redis_cluster/expan_cluster.png[扩容集群,50%]

. 修改内存及硬盘大小，点击**提交**。等待变更完成。
+
...
. 登录 {platform_name}。
. 在控制台顶部的导航菜单中，选择**产品与服务** > *数据库与缓存* > *键值数据库 Redis*，进入 Redis Cluster 管理页面。
. 在集群列表，右键单击目标集群，选择**更改云服务器类型**。
+
image::/images/cloud_service/database/redis_cluster/mdy_vm_type.png[变更云服务器类型,50%]
