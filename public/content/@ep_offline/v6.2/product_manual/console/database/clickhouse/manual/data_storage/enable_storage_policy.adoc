...
keywords: 开启对象存储策略,冷热存储,多磁盘数据存储,数据仓库,ClickHouse
weight: 10
collapsible: false
draft: false
---
...
include::../../../../../../../../_components/attention.adoc[]
对象存储服务与集群需在同一区域；若不在同一区域，可通过link:../../../../../network/border_router/[边界路由器]或 link:../../../../../network/vpc/manual/vpn/[VPN] 等方式打通网络。
...
+
image::/images/cloud_service/dwh_bi/clickhouse/enable_bucket_policy.png[启动对象存储策略]

. 配置存储策略信息，详细参数说明请参见<<_存储策略参数>>。
...
image::/images/cloud_service/dwh_bi/clickhouse/check_storge_policy.png[查看对象存储策略]
...
* 若需使用冷热存储策略，请参见link:../config_hot_to_cold_storage[配置冷热分离存储]。
