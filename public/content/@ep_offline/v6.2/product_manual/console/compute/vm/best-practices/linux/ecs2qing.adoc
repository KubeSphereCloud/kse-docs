...
. 登录阿里云控制台，点击左上角选择对象存储 OSS，初次使用同意 OSS 服务条款，进入 {platform_name}。
+
...
. 我们在{product_name}的虚机里下载该镜像。没有 wget 需要先下载安装 wget 工具，并且注意下面命令里面的两个单引号。
+
[source,shell]
----
wget ‘下载链接’
...
=== 第2步：制作{product_name}自定义镜像

. 在 {platform_name}上创建一个硬盘并挂载到下载镜像的云服务器上，注意硬盘的大小需要和原云服务器的系统盘大小一致。
+
[source,shell]
----
fdisk -l
...
需要选择一个系统镜像，我需要导入的镜像是centos7.5版本，所以选择{product_name}对应的centos7.5即可，然后提交。
...
. 镜像创建完成，我们便可以在镜像 > 自有界面看到我们刚才创建的镜像，然后基于该镜像在{product_name}上创建虚机即可。
+
