...
=== 宝塔面板使用{product_name}证书

用户在{product_name}申请了 SSL 证书，需要在宝塔中部署的网站中使用。
...
用户通过宝塔面板部署了应用程序，部署后已按照放行 8888 端口的方式在{product_name}放行了服务端口，却仍然无法正常访问服务，此时需要检查宝塔面板上的防火墙是否放行该服务端口。 
