...
用户在{product_name}购买数据库产品，购买后想要通过公网或内网去连接数据库，本文主要介绍使用{product_name}数据库集群如何进行连接。
...
=== 非{product_name}云服务器连接

非{product_name}云服务器。需要连接{product_name}云数据库。需要在 VPC 中配置云数据库端口的端口转发规则，并在 VPC 绑定的安全组中放行端口。
