...
MetaData 是{product_name}定义的一组关于云服务器运行参数的字段，主要包括云服务器 ID，云服务器 CPU 和内存配置，操作系统，挂载的硬盘，所在的子网等。

==== 通过文件写入

{product_name}可以通过将 MetaData 写入云服务器内文件的方式提供给用户，用户对文件做一定处理后即可获得 MetaData 值。

为方便用户编程时对参数值的调取，对 MetaData 文件的参数格式做了特殊规定，目前支持两种。

** JSON 格式：参数与参数值写成 JSON 格式，与目前{product_name} API 的提供方式类似。
** ENV 格式：参数与参数值写成类似于系统环境变量的形式，方便 bash 等的读取。


这两个文件，分别命名为 metadata.json 和 metadata.env，默认会放在云服务器的相应目录下（可通过自定义路径更改），用户可自行选择使用其中一种。目前 MetaData 提供的字段如下。
...
| instance_type
| String
| 云服务器类型，为{product_name}预定义的一些配置类型。
...
需要注意的是，Metadata Server 的访问有如下限制。
...
+
用户打包时需从云服务器的根目录位置开始打包，如此才可包含全路径。{product_name}系统会将文件从云服务器的根目录起解包至对应位置。
+
