...
---
在连接实例前，请确保您的云服务器已经绑定了弹性公网 IP 或者采用 NAT 转发了相应的端口，并且您的 `防火墙` 放行了相应端口。
...
* 需要绑定公网 IP。
* 若需要进行 WebSSH 远程登录，在绑定密钥时，不要勾选禁止密码登录。
...
+
若云服务器不接受您的密码，您可以将云服务器关机后link:../../mgmt_vm/reset_pw[重置云服务器的登录密码] 。
+
...
+
image::/images/cloud_service/compute/vm/manual_linux_connect_3.png[,60%]
...
如果您使用的是 Windows 操作系统，除了通过控制台以外，您也可以通过软件连接至 Linux 云服务器，常见的软件有 link:https://www.putty.org/[PuTTY] 、link:https://www.netsarang.com/zh/xshell/[Xshell]等。

include::../../../../../../../../../_components/note.adoc[]

{product_name}并不知道您的软件来源和用途，也不对其产生的任何的问题和纠纷负责。
...
. 给现有云服务器加载密钥，右键点击云服务器 ID，选择**登录设置** > **配置 SSH 密钥**，在弹出的**选择 SSH 密钥**窗口中，勾选待加载的 SSH 密钥，点击**确定**。
+
image::/images/cloud_service/compute/vm/manual_linux_connect_11_co.png[,60%]

==== 步骤三：配置公网 IP 和防火墙

在连接云服务器之前，请确保您的云服务器已经绑定了公网 IP，并放行防火墙 TCP 22 端口。
