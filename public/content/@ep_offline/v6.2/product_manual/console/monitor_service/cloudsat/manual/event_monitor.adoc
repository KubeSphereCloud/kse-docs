...
事件监控对{product_name}核心资源的部分关键操作行为进行了事件级定义，并按不同时间周期对其进行统一汇总统计与展示。除此之外，用户可对关注的事件进行告警策略的设置，在被监控事件触发后可通过{product_name}完整地监控告警服务，及时通知、定位并处理问题。


== 查看事件监控

. 用户登录{product_name}的{console_name}后，点击**产品与服务** > **运维与监控** > **监控服务** > **云监控 CloudSat**，进入**云监控 CloudSat** 主页面。
...
|产品类型
|{product_name}的资源类型，如，云服务器、硬盘、负载均衡器等
|-	
