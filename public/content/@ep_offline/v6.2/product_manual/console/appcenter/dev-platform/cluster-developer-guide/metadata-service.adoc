...
| sid
| 节点 server ID，{product_name}调度系统自动为每个节点分配的从1开始的整数。

| gid
| 节点 group ID，{product_name}调度系统自动为每个组分配的从1开始的整数，每一个节点和它的 replica 组成一个 group，即它们的 gid 相同，这个是为分片式分布式系统(多主多从，每个主和它的从为同一个组)提供的特性。

| gsid
| 节点 global server ID，{product_name}调度系统自动为每个节点分配的全球唯一的9位随机整数 ID。

| node_id
| 节点 node ID，{product_name}调度系统自动为每个节点分配的节点 ID，是一个以 cln- 开头的唯一标识，此 ID 不会变更。

| instance_id
| 节点云服务器 ID，{product_name}调度系统自动为每个节点分配的云服务器 ID，是一个以i-开头的唯一标识，此 ID 是云服务器的 hostname，每次启动都会变更，如关闭集群然后启动集群，该节点 instance ID 可能会变更。
...
| cluster_id
| 集群 ID，用户在创建应用的时候{product_name}调度系统自动生成的一个以 cl- 开头的唯一标识，如 cl-0u0a6u1j。
...
对于参数类型是 accesskey 的参数，在其原来目录下，{product_name}调度系统会添加 /access_key_id 和 /secret_access_key 两个子目录，分别存储 API 密钥ID 和私钥。
...
cmd 表示本节点需要执行的命令。开发者不需要用到这类信息，这是{product_name}调度系统转发并执行应用命令，如启动应用命令等。
...
在创建好一个集群后，登录到任意一个节点，在文件 /etc/confd/confd.toml 里找到 nodes 这一行(这个文件是{product_name}调度系统在用户创建集群的时候自动生成的)，这一行定义的是 metadata server 的 IP 地址，任取一个 IP，运行下面命令即可看到所有信息。注明：同一 VPC 里所有集群这个文件内容相同。
