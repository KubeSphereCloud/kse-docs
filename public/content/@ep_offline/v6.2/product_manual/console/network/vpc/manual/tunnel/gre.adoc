...
以下图场景为例，用户数据中心 1（ 6.6.6.6 ）作为中心节点，{product_name}（3.3.3.3）及用户数据中心 2（9.9.9.9）作为接入点，形成星型拓扑。现需要配置 GRE 隧道实现用户数据中心1 与 VPC 和另一数据中心互通。

image::/images/cloud_service/network/vpc/gre_l3_star_2.gif[]

. 参照前文内容，创建一条到数据中心 1（6.6.6.6） 的隧道，即完成了在{product_name}中的操作。
