...
draft: false
---
. 在任意可以连通两个 Elasticsearch 集群的主机上安装 elasticdump，命令如下：
+
[,bash]
----
...
. elasticdump 安装成功后，即可进行数据迁移操作，命令如下：
+
[,bash]
----
...
将 192.168.0.37 替换为原{product_name}大数据平台的 Elasticsearch 集群的任意节点的 IP 地址，将 192.168.0.24 替换为 ELK 集群的任意 Elasticsearch 节点的 IP 地址。
