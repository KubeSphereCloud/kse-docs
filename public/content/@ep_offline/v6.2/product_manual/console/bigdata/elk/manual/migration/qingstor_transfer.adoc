...
| 集群节点地址
| <原{product_name}大数据平台的 Elasticsearch=集群的某一节点的 IP 地址>需替换为具体的 IP 地址
...
| access_key
| {product_name}账号关联的 access_key

| secret_key
| {product_name}账号关联的 secret_key
...
curl -XPUT 'http://<原{product_name}大数据平台的 Elasticsearch 集群的某一节点的IP地址>:9200/_snapshot/repo-stor/migration-2019.05.13?wait_for_completion=true'
----
