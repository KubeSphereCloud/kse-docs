...
| endpoint
| s3.[region].stor.com （请根据实际情况就近选择，目前支持：pek3a, pek3b, sh1a，gd2。

| access_key
| {product_name}账号关联的access_key。

| secret_key
| {product_name}账号关联的secret_key。

| bucket
| 对象存储上bucket名称my_stor_bucket(如果不存在将创建出来)。
|===

include::../../../../../../../_components/note.adoc[]
详细参数说明请参考link:https://www.elastic.co/guide/en/elasticsearch/plugins/current/repository-s3-client.html[官方文档]。
...
. 用如下命令创建快照（该快照将会存放在之前指定的对象存储的 bucket  `my_stor_bucket` 中）。
+
[,bash]
----
...
. 通过如下命令恢复存储在对象存储的快照到 Elasticsearch 集群。
+
[,bash]
----
...
. 快照由于并没有和具体的集群信息绑定，所以也可以恢复到另一个不同的集群，用户可以用这种方法在不同集群之间通过对象存储导入导出数据。
+
