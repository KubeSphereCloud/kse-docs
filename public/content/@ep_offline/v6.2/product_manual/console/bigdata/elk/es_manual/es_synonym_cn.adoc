...
. 按照每行一组同义词、词语之间以**半角**逗号 `,` 分隔的格式准备名为 `synonym.txt` 的同义词词典文件。
+
[,bash]
----
番茄,西红柿 => 西红柿,番茄
...
详细的 synonym.txt 文件配置说明请参考link:https://www.elastic.co/guide/en/elasticsearch/reference/master/analysis-synonym-tokenfilter.html[官方文档]。

include::../../../../../../../_components/table_footer.adoc[]

. 通过如下命令在**所有** Elasticsearch 节点和 Elasticsearch 专有主节点上传词典。
+
...
. 进行同义词搜索测试。使用如下 bash 脚本测试同义词搜索功能。
+
[,bash]
----
INDEX=synonym-test
...
. 查看同义词搜索结果。如下图所示，搜索结果包含"西红柿"和"番茄"的所有文档。
+
image::/images/cloud_service/bigdata/elk/synonym_result.png[synonym_result]
+

include::../../../../../../../_components/note.adoc[]

* 对象存储为用户提供了云端可无限扩展的通用数据存储服务，具有安全可靠、简单易用、高性能、低成本等特点。 
* 用户可以将数据、日志、静态资源等多种文件类型，通过多种方式上传至对象存储中，以满足日常数据存储、归档、分析等需求。
* 为了更好的满足用户的需求，{product_name}提供了Elasticsearch、Logstash 等与对象存储的集成功能。
