...
keywords: 搜索分析,从 ELK 到 OpenSearch，SnapShot，快照方式，索引迁移,OpenSearch,搜索引擎,大数据
weight: 10
collapsible: false
draft: false
---
OpenSearch 服务支持通过 https://opensearch.org/docs/latest/opensearch/snapshot-restore/#amazon-s3[repository-s3] 插件，以 SnapShot 方式从 ELK 迁移数据到 OpenSearch。
...
. 登录 Kibana  {platform_name}，在 Dev Tools 的 Console 中分别执行以下命令。
+
[,bash]
----
GET _cat/indices
...
. 登录 QingStor  {platform_name}，即可在相应桶中查看到生成的快照文件。
+
image::/images/cloud_service/bigdata/opensearch/check_elk_snapshot.png[查看快照文件]

== 步骤 2: 在 OpenSearch 创建 snapshot-repository

登录 Dashboard  {platform_name}，在 Dev Tools 的 Console 中分别执行以下命令。
...
为恢复已被修改的索引，需关闭（close）索引编辑，才能恢复（restore）索引。 +
以恢复 `s0013` 仓库索引为例：
