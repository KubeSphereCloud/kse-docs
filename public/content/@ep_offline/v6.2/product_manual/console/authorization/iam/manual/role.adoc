...
为了控制可以接入{product_name}访问客户资源的实体，您可以通过创建 IAM 身份向应用程序或他人来委派您名下{product_name}服务和资源的访问权限。
...
+
您可以选择产品资源或{product_name}账户类型来创建身份，点击**了解更多**以查看该载体类型的描述和适用场景介绍。
+

include::../../../../../../../_components/note.adoc[]
当您为身份指定产品资源类的信任载体时，您可以在下拉框中选择指定具体的产品资源；而当您为身份指定{product_name}账户类的信任载体时，则需要对方实现将其账户 ID 提供给您。
...
如果一旦您没有在创建时关联或者在创建完后有附加新权限集合的需要，{product_name} IAM 依然提供再次为该身份添加和移除策略的操作，并支持一次性添加或移除多条策略。
...
为了让赋予身份权限的对方账户更聚焦操作，{product_name} IAM 将其可操作权限设定了双重保险，默认为对方切换身份后的账户控制台设置了 URL 可见模块，只有开通的模块才可通过 UI 控制台操作。
...
当{product_name} IAM 身份创建成功后，您可以通过下载{product_name}官方 SDK 来更新您应用中的认证代码来调用身份，账户类身份也可以通过登录 {platform_name} UI 面板执行**切换身份**操作。
