...
include::../../../../../../../_components/note.adoc[]
**预置策略**是{product_name}为满足您的日常管理需要而提供，无法删除和更改。

**应用托管策略**是由{product_name} AppCenter 应用开发者定义，您在部署相应应用时即自动生成，也是无法自主删除和更改的。
...
. 选择操作后，页面将指引您为选择的操作设定权限作用范围，默认是作用于**全部资源**，如需将权限范围圈定到部分特定资源上，点击**特定资源**并配置link:../../faq/qrn[资源标识符 QRN] 即可。{product_name} IAM 会根据您所选的操作自动关联出操作所需的所有资源，如下图。
+
...
include::../../../../../../../_components/note.adoc[]
当您自行使用 JSON 编辑来创建策略时，需要满足{product_name}的格式规则，否则可能会创建失败或者偏离预期的策略效果。
...
策略是一系列操作权限的集合，将其引用到身份即可控制该身份访问{product_name}的接入。一个身份上可以附加多条策略，一个策略也可引用到多个身份。{product_name} IAM 支持针对策略操作一次性引用到多个身份。
