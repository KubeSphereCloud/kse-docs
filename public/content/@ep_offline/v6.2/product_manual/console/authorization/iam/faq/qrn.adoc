...
{product_name}资源名称（QRN）被用来作为资源的唯一标识。当您需要在全局环境（比如 link:../../manual/policy[IAM 策略]）中明确指定一项资源时，我们要求使用 QRN 。
...
以上是 QRN 的一般格式，具体组成部分和值取决于{product_name}服务。
要使用 QRN，请将 *QRN 格式* 中的**青色斜体**文本替换为实际信息。

include::../../../../../../../_components/table_footer.adoc[]

* *全局资源名称(qrn)*：表示是{product_name}的云资源。该字段是必填项，且固定为 ``qrn``。
* *平台分区(partition)*：目前作为保留字段。该字段是必填项，且固定为 ``qingcloud``。
* *服务(service)*：{product_name}（例如：弹性云服务器[ecm]、虚机私有云[vpc] 等）的产品命名空间，该字段是必填项，相关服务列表请参阅link:../supported_services/#_服务类别[服务类别]。
* *区域(region)*：资源所在的区域或可用区（例如：北京3区[``pek3``]、亚太2区-A[``ap2a``]等）。一些全局资源的 QRN 不需要指定区域，因此该字段可能会被省略，相关区域列表请参阅link:../supported_services/#_区域和可用区[区域列表]。
* *账号标识符(account-id)*：该资源的所属{product_name}账号 id（例如：usr-xxxxxx），该字段是必填项。
