...
. 向您需要授权的人获取到他的{product_name}账户 ID 。例如对方账户为 `claudiawang@yunify.com` ，ID 为 `usr-2Ly2c31J`。
+
...
根据上述操作步骤，由于您给他人赋予的身份具备虚拟专用网服务的完全权限，则他人登录 {platform_name}后可通过切换身份成功帮您创建私有网络，但不可访问您的其他服务例如云服务器。
