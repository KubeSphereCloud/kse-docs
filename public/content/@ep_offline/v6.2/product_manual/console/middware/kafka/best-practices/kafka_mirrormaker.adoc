...
keywords: 云计算,大数据,消息队列,中间件,Kafka,MirrorMaker,跨集群迁移
weight: 10
draft: false
---
...
您只需要通过简单的 consumer 配置和 producer 配置，然后启动 MirrorMaker，即可实现准实时的数据同步。更多详细信息可以参见官方文档 https://kafka.apache.org/documentation/[Kafka-MirrorMaker]。
...
+
image::/images/cloud_service/middware/kafka/auto_create_topics_enable_true.png[设置参数]

== 操作步骤

登录集群 A 的客户端节点，执行以下操作。

. 创建消费配置文件 consumer.config，并配置消费者参数。
+
[source]
----
 // A 集群的 broker 列表
...
. 创建生产者配置文件 producer.config，并配置生产者参数。
+
[source]
----
 // B 集群的 broker 列表
...
. 执行以下命令，运行 kafka-mirror-maker.sh 脚本文件进行数据迁移。
+
[source]
----
