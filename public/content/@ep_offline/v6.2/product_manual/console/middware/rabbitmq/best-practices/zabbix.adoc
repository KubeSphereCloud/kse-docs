...
keywords: Zabbix 监控,zabbix server,RabbitMQ
weight: 35
collapsible: false
draft: false
---
...
image::/images/cloud_service/middware/rabbitmq/zabbix_arh_rabbitmq.png[zabbix,70%]
...
image::/images/cloud_service/middware/rabbitmq/zabbix_rabbitmq_user.png[RabbitMQ 用户]

== 前提条件

* 已获取 {platform_name}登录账号和密码，且已获取集群操作权限。
...
* 若安装 Zabbix 的服务器与 RabbitMQ 网络不通，可通过link:../../../../network/border_router/[边界路由器]或 link:../../../../network/vpc/manual/vpn/vpn_intro/[VPN] 等方式打通网络。不建议通过**端口转发**的方式将服务暴露到公网，以免造成 RabbitMQ 关键信息暴露等风险。
...
. 在**服务端口信息**模块，查看待监控集群的 VIP 地址。
+
image::/images/cloud_service/middware/rabbitmq/zabbix_rabbitmq_vip.png[查看 RabbitMQ 集群 VIP,70%]
...
. 在 **Hosts** 页签，配置 RabbitMQ 的集群 VIP 为监控主机。
+
    * **Host name** 自定义主机名称，例如 `rabbitmq-doc`。
    * **Templates** 选择模版，您可以选择系统自带且适用于 RabbitMQ 集群的模板，此处以模板 `RabbitMQ cluster by HTTP` 为例；您也可以在 **Configuration** > **Templates** 界面自定义模板，详细操作请参见 link:https://www.zabbix.com/documentation/6.0/zh/manual/config/templates/template[Zabbix]。
...
+
image::/images/cloud_service/middware/rabbitmq/zabbix_rabbitmq_create_host01.png[创建 Host]
...
include::../../../../../../../_components/table_footer.adoc[]
+
image::/images/cloud_service/middware/rabbitmq/zabbix_rabbitmq_create_host02.png[配置主机宏]

. 点击 **Add**，创建主机。
+
待主机的 **Status** 为 `Enabled` 表示监控配置成功，即可查看采集的最新数据和监控图。
+
image::/images/cloud_service/middware/rabbitmq/zabbix_status.png[配置成功状态]
+
...
. 点击主机名称所在行的 **Items**、**Triggers**、**Graphs**、**Discovery**，可查看 Zabbix Server 对 RabbitMQ 集群支持的监控项、触发器、数据图表等详细监控信息。
+
image::/images/cloud_service/middware/rabbitmq/zabbix_items.png[查看]
+
image::/images/cloud_service/middware/rabbitmq/zabbix_items_detail.png[查看监控项]
...
可查看监控图表。
+
image::/images/cloud_service/middware/rabbitmq/zabbix_graphs.png[查看]

. 点击右上角时间可筛选对应时间段的监控状况。
+
image::/images/cloud_service/middware/rabbitmq/zabbix_graphs01.png[查看图表]
+
