...
function getAllAdocFiles() {
  logger.info(
    'Get all adoc files by dir',
    PDF_CONTENT_DIR,
    globSync(`${PDF_CONTENT_DIR}/**/*.adoc`).length,
  );
  ...
  // 只有标记了 `study_section` 的 section 才会被导出吗？
  // 那岂不就是缺失了一部分？为什么会有这个逻辑呢？
  const studySections = listFiles.filter((item) => item.meta.study_section).map((item) => path.dirname(item.filePath));

  const nextStudySections = studySections
    .map((section) => {
  ...
  const sections = uniq(PDF_CONTENT_ONLY_DIR).length
    ...
    : uniq(
        [
          ...
          ...nextStudySections,
          // 命令行自定义目录
          ...(PDF_CONTENT_VERSIONS_DIR || []).filter((v) => studySections.find((section) => v.startsWith(section))),
